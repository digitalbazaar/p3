var config = require('../payswarm.config');

// financial defaults
config.financial.defaults.account = {
  '@type': 'Account',
  // demo with $10
  'com:balance': '10.0000000',
  'com:escrow': '0.0000000',
  'com:currency': 'USD',
  'psa:status': 'active',
  'psa:privacy': 'private'
};
config.financial.defaults.paymentTokens.push({
  // demo payment token source
  '@type': 'ccard:CreditCard',
  'rdfs:label': 'My Visa',
  'com:gateway': 'Test',
  'ccard:brand': 'ccard:Visa',
  'ccard:number': '4111111111111111',
  'ccard:expMonth': '11',
  'ccard:expYear': '16',
  'ccard:cvm': '111',
  'ccard:address': {
    'vcard:fn': 'Billing Name',
    'vcard:street-address': '1 Billing Lane',
    'vcard:locality': 'Locality',
    'vcard:region': 'Region',
    'vcard:postal-code': '12345',
    'vcard:country-name': 'US'
  }
});

// profiles
config.profile.profiles.push({
  '@id': config.authority.baseUri + '/profiles/authority',
  '@type': 'ps:Profile',
  'psa:slug': 'authority',
  'foaf:mbox': 'authority@payswarm.com',
  'rdfs:label': 'PaySwarm Development Authority',
  'psa:password': 'password',
  'psa:role': [
    config.authority.baseUri + '/roles/financial_administrator',
    config.authority.baseUri + '/roles/identity_administrator',
    config.authority.baseUri + '/roles/role_administrator',
    config.authority.baseUri + '/roles/system_administrator',
    config.authority.baseUri + '/roles/profile_administrator'
  ]
});
config.profile.profiles.push({
  '@id': config.authority.baseUri + '/profiles/dev',
  '@type': 'ps:Profile',
  'psa:slug': 'dev',
  'foaf:mbox': 'dev@payswarm.com',
  'rdfs:label': 'Dev',
  'psa:password': 'password',
  'psa:role': [
    config.authority.baseUri + '/roles/profile_registered',
    config.authority.baseUri + '/roles/identity_manager',
    config.authority.baseUri + '/roles/financial_manager']
});
config.profile.profiles.push({
  '@id': config.authority.baseUri + '/profiles/customer',
  '@type': 'ps:Profile',
  'psa:slug': 'customer',
  'foaf:mbox': 'customer@payswarm.com',
  'rdfs:label': 'Customer',
  'psa:password': 'password',
  'psa:role': [
    config.authority.baseUri + '/roles/profile_registered',
    config.authority.baseUri + '/roles/identity_manager',
    config.authority.baseUri + '/roles/financial_manager']
});
config.profile.profiles.push({
  '@id': config.authority.baseUri + '/profiles/vendor',
  '@type': 'ps:Profile',
  'psa:slug': 'vendor',
  'foaf:mbox': 'vendor@payswarm.com',
  'rdfs:label': 'Vendor',
  'psa:password': 'password',
  'psa:role': [
    config.authority.baseUri + '/roles/profile_registered',
    config.authority.baseUri + '/roles/identity_manager',
    config.authority.baseUri + '/roles/financial_manager'
 ]
});

// identities
config.identity.identities.push({
  '@id': config.authority.baseUri + '/i/authority',
  'ps:owner': config.authority.baseUri + '/profiles/authority',
  'psa:slug': 'authority',
  'rdfs:label': 'PaySwarm Authority'
});
config.identity.identities.push({
  '@id': config.authority.baseUri + '/i/dev',
  '@type': 'ps:PersonalIdentity',
  'ps:owner': config.authority.baseUri + '/profiles/dev',
  'psa:slug': 'dev',
  'rdfs:label': 'Dev'
});
config.identity.identities.push({
  '@id': config.authority.baseUri + '/i/customer',
  '@type': 'ps:PersonalIdentity',
  'ps:owner': config.authority.baseUri + '/profiles/customer',
  'psa:slug': 'customer',
  'rdfs:label': 'Customer'
});
config.identity.identities.push({
  '@id': config.authority.baseUri + '/i/vendor',
  '@type': 'ps:VendorIdentity',
  'ps:owner': config.authority.baseUri + '/profiles/vendor',
  'psa:slug': 'vendor',
  'rdfs:label': 'Vendor',
  'foaf:homepage': 'http://example.com/vendor',
  'dc:description': 'The default PaySwarm Vendor'
});

// keys
config.identity.keys.push({
  publicKey: {
    '@id': config.authority.baseUri + '/i/authority/keys/1',
    '@type': 'sec:Key',
    'ps:owner': config.authority.baseUri + '/i/authority',
    'rdfs:label': 'Key 1',
    'sec:publicKeyPem': '-----BEGIN PUBLIC KEY-----\\nkQIDAQAB\n-----END PUBLIC KEY-----\n'
  },
  privateKey: {
    '@type': 'sec:Key',
    'ps:owner': config.authority.baseUri + '/i/authority',
    'rdfs:label': 'Key 1',
    'sec:publicKey': config.authority.baseUri + '/i/authority/keys/1',
    'sec:privateKeyPem': '-----BEGIN RSA PRIVATE KEY-----\\njQZJvKo6cWiuyPAZFh1TNZsQUaLnXNLJUWdIjXqKI4aQ9fdTi5E+\n-----END RSA PRIVATE KEY-----\n'
  }
});

// accounts
config.financial.accounts.push({
  '@id': config.authority.baseUri + '/i/dev/accounts/primary',
  '@type': 'com:Account',
  'ps:owner': config.authority.baseUri + '/i/dev',
  'psa:slug': 'primary',
  'rdfs:label': 'Primary Account',
  'com:currency': 'USD'
});
config.financial.accounts.push({
  '@id': config.authority.baseUri + '/i/customer/accounts/primary',
  '@type': 'com:Account',
  'ps:owner': config.authority.baseUri + '/i/customer',
  'psa:slug': 'primary',
  'rdfs:label': 'Primary Account',
  'com:currency': 'USD'
});
config.financial.accounts.push({
  '@id': config.authority.baseUri + '/i/vendor/accounts/primary',
  '@type': 'com:Account',
  'ps:owner': config.authority.baseUri + '/i/vendor',
  'psa:slug': 'primary',
  'rdfs:label': 'Primary Account',
  'com:currency': 'USD'
});
