...
// increment settleAfter by 1 minute when status check result is pending
config.financial.transaction.statusSettleAfterIncrement = 1000 * 60;
// maximum number of status checks before giving up
config.financial.transaction.maxStatusChecks = 5;
...
// consider an unverified payment token idle after 1 hour (needs attention)
config.financial.paymentToken.worker.tokenIdleTimeout = 1000 * 60 * 60;
