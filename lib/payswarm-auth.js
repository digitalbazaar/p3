...
var limiter = require(__dirname + '/payswarm-auth/payswarm.limiter');
var passport = require('passport');
var config = require('./payswarm.config');
var program = require('commander');
var connect = require('connect');
var loggers = require(__dirname + '/payswarm-auth/payswarm.loggers');
var docs = require(__dirname + '/payswarm-auth/payswarm.docs');
...
api.start = function() {
  ...
  i18n.configure({
    ...
    locales: config.website.locales,
  ...
  loggers.init(function(err) {
    ...
    if(cluster.isMaster) {
      ...
      for(var i = config.website.content.length - 1; i >= 0; --i) {
        // use the English i18n directory for the base static files
        var p = path.resolve(config.website.content[i] + '/en/static');
