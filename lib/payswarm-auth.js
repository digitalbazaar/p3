  ...
  loggers.init(function(err) {
    ...
    if(cluster.isMaster) {
      ...
      app.server.on('listening', function() {
        startupState.httpsListening = true;
        _loadModules(startupState);
      });
      http.on('listening', function() {
        startupState.httpListening = true;
        _loadModules(startupState);
      });
...
function _loadModules(state) {
  if(state.httpListening && state.httpsListening && !state.ready) {
    state.ready = true;
    if(config.environment !== 'development') {
      process.setgid(config.app.user.groupId);
      process.setuid(config.app.user.userId);
    ...
    async.forEachSeries(config.modules, function(mod, callback) {
      ...
      mod = require(mod);
      mod.init(state.app, function moduleLoaded(err) {
        if(!err) {
