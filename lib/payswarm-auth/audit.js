        ...
        cursor.each(function(err, record) {
          ...
          if(record !== null) {
            ...
            if(!t.settled && !t.voided) {
              // skip, active
              logger.verbose(sprintf('active %s', info.type), {id: t.id});
              stats.active[info.stats].count += 1;
              stats.active[info.stats].amount =
                stats.active[info.stats].amount.add(t.amount);
            } else if(t.voided) {
              // skip, voided
              logger.verbose('voided', {id: t.id});
              stats.voided[info.stats].count += 1;
              stats.voided[info.stats].amount =
                stats.voided[info.stats].amount.add(t.amount);
            } else if(info.type === 'contract' || info.type === 'transfer') {
              // settled contract or transfer
              logger.verbose(sprintf('settled %s', info.type), {id: t.id});
              stats.settled[info.stats].count += 1;
              stats.settled[info.stats].amount =
                stats.settled[info.stats].amount.add(t.amount);
              ...
              // settled deposit
              logger.verbose('settled deposit', {id: t.id});
              stats.settled[info.stats].count += 1;
              stats.settled[info.stats].amount =
                stats.settled[info.stats].amount.add(t.amount);
              t.transfer.forEach(function(xfer) {
              ...
              // settled withdrawal
              logger.verbose('settled withdrawal', {id: t.id});
              stats.settled[info.stats].count += 1;
              stats.settled[info.stats].amount =
                stats.settled[info.stats].amount.add(t.amount);
              t.transfer.forEach(function(xfer) {
                ...
                // reverse the xfer
                var src = data.accounts[xfer.source];
                src.balance = src.balance.subtract(xfer.amount);

                // track external xfer
                var dest = data.external[xfer.destination];
                // if not found, create it
                if(!dest) {
