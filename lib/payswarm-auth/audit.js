...
bedrock.config.audit = bedrock.config.audit || {};

bedrock.events.on('bedrock-mongodb.ready', function(callback) {
  // init financial modules
  //payswarm.financial.init(null, callback);
  brDatabase.openCollections(bedrock.config.audit.collections, callback);
});
    ...
    db.transaction.find(q, {
      ...
      cursor.each(function(err, record) {
        ...
        function _process(status) {
          if(_log.debug) {
            logger.debug(sprintf('%s %s', status, info.type), {
              transaction: t.id,
              amount: t.amount
            });
          }
        ...
        if(!t.settled && !t.voided) {
          ...
          t.transfer.forEach(function(xfer) {
            ...
            if(!src) {
              if(_log.debug) {
                logger.debug('found external deposit source', {
                  id: xfer.source
                });
              }
          ...
          t.transfer.forEach(function(xfer) {
            ...
            if(!dest) {
              ...
              if(!dest) {
                if(_log.debug) {
                  logger.debug('found external withdrawal destination', {
                    id: xfer.destination
                  });
                }
...
api.validate = function(options, query, callback) {
    ...
    async.waterfall([
      ...
      function(cursor, callback) {
        ...
        cursor.each(function(err, record) {
          ...
          //if(_log.debug) {
          //  logger.debug('found ' + collection + ':', record);
          //}
  ...
  function _ldCheck(ld, type) {
    ...
    assert(ld['@context'] === payswarm.constants.PAYSWARM_CONTEXT_V1_URL, {
      id: ld.id,
  ...
  function _sysStatusCheck(ld) {
    assert('sysStatus' in ld, {
      id: ld.id,
      message: 'missing sysStatus'
    });
    assert(typeof ld.sysStatus === 'string', {
      id: ld.id,
      message: 'non-string sysStatus'
    });
  }

  function _accountRegulationCheck(obj) {
    // FIXME: temporary check until regulation framework is complete
    if(obj.owner === bedrock.config.authority.id) {
      // ensure owner has stored value flag
      // assume its value was set correctly
      assert('sysAllowStoredValue' in obj, {
        id: obj.id,
        message: 'authority account missing sysAllowStoredValue'
      });
    }
  }
  ...
  async.waterfall([
    function(callback) {
      ...
      _validateCollection({
        ...
        validate: function(obj) {
          ...
          _sysStatusCheck(obj.account);
          _accountRegulationCheck(obj.account);
        }
      }, callback);
    },
    function(callback) {
    ...
    function(callback) {
      ...
      _validateCollection({
        collection: 'identity',
        validate: function(obj) {
          _ldCheck(obj.identity, 'Identity');
          _sysStatusCheck(obj.identity);
        }
      }, callback);
    },
    function(callback) {
    ...
    function(callback) {
      ...
      _validateCollection({
        ...
        validate: function(obj) {
          ...
          _sysStatusCheck(obj.paymentToken);
