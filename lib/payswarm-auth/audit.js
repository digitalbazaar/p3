...
api.init = function(options, collections, callback) {
  async.waterfall([
    function(callback) {
      // environment check
      if(bedrock.config.workers !== 1) {
        callback(new Error('Auditing only supports 1 worker.'));
      }
      callback();
    },
    function(callback) {
      // init database
      brDatabase.init(null, callback);
    },
    function(callback) {
      // init financial modules
      //payswarm.financial.init(null, callback);
      brDatabase.openCollections(collections, callback);
    }
  ], callback);
};
