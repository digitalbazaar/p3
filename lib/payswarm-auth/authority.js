...
api.getAuthorityKeyPair = function(actor, callback) {
  var authorityId = bedrock.config.authority.id;
  async.waterfall([
    function(callback) {
    ...
    function(callback) {
      brDatabase.collections.publicKey.findOne(
        {owner: brDatabase.hash(authorityId), 'publicKey.sysStatus': 'active'},
...
api.decryptMessage = function(actor, msg, decryptKey, callback) {
  ...
  async.auto({
    getDecryptKey: function(callback) {
      ...
      // encrypted with
      brIdentity.getIdentityPublicKey({id: msg.publicKey}, actor,
