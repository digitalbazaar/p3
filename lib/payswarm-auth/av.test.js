...
api.validateAddress = function(address, callback) {
  ...
  var out = payswarm.tools.clone(bedrock.config.addressValidator.test.address);
  out.type = 'Address';
  out.label = payswarm.tools.clone(address.label) || 'Unnamed';
  out.name = payswarm.tools.clone(address.name) || 'Full Name';
