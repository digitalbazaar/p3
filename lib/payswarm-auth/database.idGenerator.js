...
api.DistributedIdGenerator.prototype.generateId = function(callback) {
  ...
  async.waterfall([
    function(callback) {
      // no current ID or maximum ID reached, reload IDs
      if(self.currentId === null || self.currentId.isMax()) {
        return _loadIds(self, callback);
      }
      ...
      payswarm.db.collections.distributedId.findAndModify(
        ...
        function(err, record) {
          ...
          query.$or = [
            ...
            {globalKey: {$lt: idGenerator.globalId.toHex()}}
          ];
          var update = {$set: {}};
          ...
          payswarm.db.localCollection.findAndModify(
            ...
            function(err, result) {
              // error or new global ID and local ID set by this process
              if(err || result) {
                return callback(err);
              }
