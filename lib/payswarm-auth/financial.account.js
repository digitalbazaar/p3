...
bedrock.events.on('bedrock-mongodb.ready', function(callback) {
  async.waterfall([
    function(callback) {
      ...
      brDatabase.openCollections(['account'], callback);
    },
    function(callback) {
    ...
    function(callback) {
      ...
      async.forEachSeries(
        bedrock.config.financial.accounts,
        function(a, callback) {
...
bedrock.events.on(EVENT_CREDIT_PAYOFF, function(event) {
  logger.verbose('got unbacked credit payoff event', event);
  var options = {};
  if(event && event.details && event.details.accountId) {
    options.id = event.details.accountId;
  } else {
    options.reschedule =
      bedrock.config.financial.account.worker.schedule;
  }
  ...
  async.waterfall([
    ...
    function(callback) {
      brDatabase.collections.account.find(
        {owner: brDatabase.hash(identityId)}).toArray(callback);
    }
  ], callback);
};
...
api.createAccountCreditLine = function(actor, id, options, callback) {
  async.waterfall([
    ...
    function(callback) {
      options = bedrock.util.extend({}, options, {create: true});
  ...
  async.auto({
    ...
    checkToken: ['getToken', 'getAccount', function(callback, results) {
      ...
      if(!jsonld.hasValue(paymentToken, 'paymentMethod', 'CreditCard')) {
        return callback(new BedrockError(
          ...
          'example, a debit card.', MODULE_NS + '.InvalidBackupSource', {
            backupSource: backupSourceId,
            httpStatusCode: 400,
            public: true
          }));
    ...
    updateToken: ['updateAccount', function(callback) {
      ...
      brDatabase.collections.paymentToken.update({
        id: brDatabase.hash(backupSourceId),
...
api.reorderAccountBackupSources = function(actor, accountUpdate, callback) {
  async.waterfall([
    ...
    function(callback) {
      brDatabase.collections.account.update({
        id: brDatabase.hash(accountUpdate.id),
...
api.setIdentityRegulatoryAddress = function(actor, options, callback) {
  ...
  var idHash = brDatabase.hash(options.id);
  ...
  async.auto({
    ...
    updateIdentity: ['updateActiveAccounts', function(callback) {
      brDatabase.collections.identity.update(
        ...
        brDatabase.writeOptions, function(err, result) {
          if(err) {
            return callback(err);
          }
          if(result.result.n === 0) {
...
function _createAccount(account, options, callback) {
  ...
  var defaults = bedrock.config.financial.defaults;
  account = bedrock.util.extend(true, {}, defaults.account, options, account);
  logger.verbose('creating account', account);

  // allow initial balance if configured
  var balance = null;
  if(bedrock.config.financial.allowInitialBalance) {
  ...
  async.waterfall([
    ...
    function(callback) {
      ...
      var record = {
        id: brDatabase.hash(account.id),
        owner: brDatabase.hash(account.owner),
        incoming: {},
        outgoing: {},
        credit: {
      ...
      brDatabase.collections.account.insert(
    ...
    function(record, callback) {
      if(!balance || balance.isZero() || !account.sysAllowStoredValue ||
        account.owner === bedrock.config.authority.id) {
      ...
      async.waterfall([
        function(callback) {
          var deposit = {
            ...
            source: bedrock.config.financial.devPaymentToken
          };
        ...
        function(identity, callback) {
          if(!identity.sysEmailVerified) {
            return callback(new BedrockError(
              ...
              {email: identity.email, httpStatusCode: 400, public: true}));
          }
          callback(null, record);
        }
      ], callback);
    ...
    function(record, callback) {
      ...
      if(creditBackedAmount.compareTo(originalCreditBackedAmount) !== 0) {
        ...
        if(wasBelowThreshold && nowMeetsThreshold) {
          ...
          var due = new Date();
          due = bedrock.util.w3cDate(due.setTime(due.getTime() + delta));
  ...
  async.waterfall([
    ...
    function(records, cb) {
      ...
      if(newTokenHashes.length === 0) {
        ...
        // only set unbacked credit token hashes if some credit isn't backed
        var creditLimit = new Money(account.creditLimit || 0);
        var creditBackedAmount = new Money(account.creditBackedAmount || 0);
        if(creditLimit.compareTo(creditBackedAmount) > 0) {
      ...
      brDatabase.collections.account.update(
        query, update, brDatabase.writeOptions, function(err, result) {
          if(err) {
            return cb(err);
          }
...
function _runWorker(options, callback) {
  ...
  // worker
  var now = new Date();
  var expiration = bedrock.config.financial.account.worker.expiration;
  ...
  async.until(function() {return done;}, function(loop) {
    async.waterfall([
      ...
      function(record, info, callback) {
        ...
        async.whilst(function() {return sources.length > 0;}, function(next) {
          ...
          brDatabase.collections.account.update({
            id: brDatabase.hash(record.account.id),
            creditPaymentDue: {$lte: now},
            'credit.lastPayoffFailed': null,
            worker: workerId
          }, {
    ...
    if(err) {
      logger.error('account worker encountered error', {error: err});
    }
    logger.verbose('account worker (' + workerId + ') finished.');
