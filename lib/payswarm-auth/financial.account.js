    ...
    function(callback) {
      ...
      payswarm.db.collections.account.update(
        ...
        {$set: payswarm.db.buildUpdate(
          account, 'account', {
            include: ['account.label', 'account.psaPublic']
          })
...
api.addAccountBackupSource = function(actor, id, backupSourceId, callback) {
  async.auto({
    ...
    buildAccountUpdate: ['checkToken', function(callback, results) {
      ...
      // return update as is if credit line is fully backed
      // Note: assumes no race condition exists where more free credit
      // can be added to an existing credit line... otherwise extra checks
      // will have to be made before enabling a credit line
      var account = results.getAccount;
      ...
      async.waterfall([
        function(callback) {
          payswarm.identity.getIdentity(
            actor, account.owner, function(err, identity) {
              callback(err, identity);
            });
        },
        function(identity, callback) {
          payswarm.profile.getProfile(
            actor, identity.owner, function(err, profile) {
    ...
    updateAccount: ['buildAccountUpdate', function(callback, results) {
      payswarm.db.collections.account.update({
        id: payswarm.db.hash(id),
        'account.backupSource': {$nin: [backupSourceId]}
      }, results.buildAccountUpdate, payswarm.db.writeOptions, function(err) {
        if(err && payswarm.db.isDuplicateError(err)) {
  ...
  async.waterfall([
    ...
    function(callback) {
      ...
      if('minAmount' in options) {
        update.$set['account.psaMinInstantTransfer'] =
          options.minAmount.toString();
      }
      payswarm.db.collections.account.update(
...
function _updateCreditLine(id, options, callback) {
  ...
  async.waterfall([
    ...
    function(record, callback) {
      ...
      // skip step if backed amount change >= credit limit change
...
function _checkBackupSources(id, options, callback) {
  ...
  async.waterfall([
    ...
    function(records, cb) {
      ...
      // only update account where backup sources haven't changed
      var query = {
        id: payswarm.db.hash(id),
