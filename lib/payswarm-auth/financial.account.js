 ...
 *          [currentLimit] the current credit limit (Money) (to avoid duplicate
 *            changes).
 *          [amount] the Money amount to change the credit limit by (negative to
 *            decrease the credit limit).
 *          [currentBackedAmount] the current backed credit amount (Money) (
  ...
  async.waterfall([
    ...
    function(record, callback) {
      ...
      if('amount' in options) {
        // ensure expected current limit is correct
        if('currentLimit' in options &&
          creditLimit.compareTo(options.currentLimit) !== 0) {
          return callback(new PaySwarmError(
            'Could not update credit limit; current credit limit has changed.',
            MODULE_TYPE + '.CreditLimitChangeConflict',
      ...
      if('backedAmount' in options) {
        // ensure expected current backed amount is correct
        if('currentBackedAmount' in options &&
          creditBackedAmount.compareTo(options.currentBackedAmount) !== 0) {
          return callback(new PaySwarmError(
            'Could not update credit limit; current credit backed amount ' +
            'has changed.', MODULE_TYPE + '.CreditLimitChangeConflict',
