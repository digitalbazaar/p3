 ...
 *          refreshInterval the new refresh interval to use (will also cause
...
function _sanitizeBudget(budget, oldBudget) {
  if(!oldBudget) {
    ...
    // if refresh interval is not specified, set to forever by using only
    // start date
  ...
  async.forEachSeries(records, function(record, callback) {
    ...
    if(interval.length === 3) {
      ...
      if(interval[0].length > 1) {
        // budget expires after specified number of repeats
        repeats = parseInt(substr(interval[0], 1)) || 0;
        var end = start + (repeats * duration);
        expired = (end <= now);
      }

      if(!expired) {
        // not expired, see if refresh is required
        ...
        if(end <= now) {
          ...
          budget.psaRefreshInterval +=
            ['', payswarm.tools.w3cDate(start), interval[2]].join('/');
        }
      }
    }
    // startDate/endDateOrDuration, no refresh
    else if(interval.length === 2) {
      var start = +new Date(interval[1]);
      var end;

      // get end date via duration
      if(iso8601.Period.isValid(interval[2])) {
        var duration = iso8601.Period.parseToTotalSeconds(interval[2]) * 1000;
        end = +start + duration;
      }
      // simple end date
      else {
        end = +new Date(interval[2]);
      }

      expired = (end <= now);
    }
    // else no refresh or expiration
    ...
    if(refresh) {
      return _atomicUpdateBalance(
        budget.id, {refreshInterval: budget.psaRefreshInterval}, function(err) {
        if(!err) {
  ...
  async.waterfall([
    ...
    function(record, callback) {
      ...
      if('refreshInterval' in options) {
        ...
        update.$set['budget.psaRefreshInterval'] = options.refreshInterval;
      }
      else {
        // add amount
