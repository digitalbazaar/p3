...
api.updateBudget = function(actor, budgetUpdate, callback) {
  async.auto({
    ...
    sanitizeBudget: ['checkLimits', function(callback) {
...
function _sanitizeBudget(budget, update) {
  if(!update) {
    ...
    budget.balance = new Money(budget.balance);
    budget.amount = new Money(budget.amount);
    if(budget.balance.compareTo(budget.amount) > 0) {
      budget.balance = budget.amount;
    }

    // set max per use to amount if not set
    if(!('psaMaxPerUse' in budget)) {
      budget.psaMaxPerUse = budget.amount;
    }
    else {
      budget.psaMaxPerUse = new Money(budget.psaMaxPerUse);
    }

    // normalize monetary amounts
    budget.balance = budget.balance.toString();
    budget.amount = budget.amount.toString();
    budget.psaMaxPerUse = budget.psaMaxPerUse.toString();
    ...
    // normalize monetary amounts
    if('amount' in budget) {
      budget.amount = new Money(budget.amount).toString();
    }
    if('psaMaxPerUse' in budget) {
      budget.psaMaxPerUse = new Money(budget.psaMaxPerUse).toString();
    }
  }
