  ...
  async.auto({
    ...
    getAssetProvider: ['getAsset', function(callback, results) {
      ...
      payswarm.identity.getIdentity(null, id, function(err, identity) {
        ...
        if('homepage' in identity) {
          assetProvider.homepage = identity.homepage;
        }
        if('description' in identity) {
          assetProvider.description = identity.description;
        }
        // Note: add asset provider address later
...
api.finalizeContract = function(actor, contract, options, callback) {
  ...
  async.auto({
    ...
    getAcquirer: ['checkPermission', function(callback) {
    ...
    checkSourceAccount: ['checkPermission', function(callback) {
    ...
    createTransfers: ['generateId', function(callback, results) {
      contract.id = results.generateId;

      // combine listing and contract payees
      var payees = jsonld.getValues(contract.listing, 'payee');
      var contractPayees = jsonld.getValues(contract, 'payee');
      payees = payees.concat(contractPayees);

      // create transfers
      payswarm.tools.createTransfers(
        contract, options.acquirerAccountId, payees, callback);
    }]
  }, function(err, results) {
    if(err) {
      return callback(err);
...
api.processContract = function(actor, contract, options, callback) {
  ...
  async.auto({
    ...
    getAssetProvider: ['checkPermission', function(callback) {
      payswarm.identity.getIdentity(
        null, contract.assetProvider.id, function(err, identity) {
          if(err) {
            return callback(new PaySwarmError(
              'The Asset provider\'s information was not found at the ' +
              'provided PaySwarm Authority.',
              MODULE_TYPE + '.AssetProviderNotFound', {
                'public': true,
                authority: payswarm.config.authority.id,
                assetProvider: contract.assetProvider.id
              }, err));
          }
          callback(null, identity);
        });
    }],
    addAddress: ['getAssetProvider', function(callback, results) {
      var identity = results.getAssetProvider;
      if(!('address' in identity) || identity.address.length === 0) {
        return callback(new PaySwarmError(
          'The Asset provider does not have a primary address set.',
          MODULE_TYPE + '.AddressNotFound', {'public': true}));
      }
      contract.assetProvider.address = identity.address[0];
      callback();
    }],
    authorize: ['addAddress', function(callback) {
      // attempt to authorize contract
      payswarm.financial.authorizeTransaction(
        contract, options.duplicateQuery, function(err) {
          // clear asset provider address to hide from buyer
          delete contract.assetProvider.address;
          callback(err);
        });
    }],
