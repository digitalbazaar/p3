  ...
  async.auto({
    ...
    checkAccountAccess: ['checkPayeeGroups', function(callback) {
      ...
      async.forEachSeries(payees, function(payee, callback) {
        // force payees into gateway group
        jsonld.addValue(
    ...
    checkAccountCurrency: ['checkAccountAccess', function(callback, results) {
      // check all destination account currencies match
      var currency = null;
      async.forEachSeries(results.checkAccountAccess,
        ...
        if(!currency) {
          ...
          return callback(new PaySwarmError(
            'Different destination account currencies.',
    ...
    sign: ['createTransfers', 'checkAccountCurrency',
      function(callback, results) {
      ...
      // FIXME: this is checking deposit amount + fees
      var currency = results.checkAccountCurrency;
      var amount = new payswarm.money.Money(deposit.amount);
