 ...
 *          [triggeredBy] a transaction ID to use to add to the deposit to
 *            indicate it was triggered by another transaction.
 *          [triggerReason] the reason for triggering the deposit (given if
 *            triggeredBy is given).
 * @param callback(err, deposit) called once the operation completes.
 */
  ...
  async.auto({
    ...
    verify: ['getPublicKey', function(callback, results) {
      ...
      payswarm.security.verifyJsonLd(deposit, key, function(err, verified) {
        ...
        // add trigger info if given
        if(options.triggeredBy) {
          deposit.triggeredBy = options.triggeredBy;
          if(options.triggerReason) {
            deposit.triggerReason = options.triggerReason;
          }
        }
        callback();
      });
    ...
    chargeSource: ['getProfile', function(callback, results) {
      // if deposit was triggered by insufficient funds, use HOLD+CAPTURE (only
      // hold funds here, to be captured when settling)
      var gatewayOp = gateway.chargeDepositSource;
      if(deposit.triggerReason === 'InsufficientFunds') {
