...
api.verifyTransaction = function(transaction, options, callback) {
  ...
  async.auto({
    ...
    profile: ['identity', function(callback, results) {
      // get profile without permission check
      payswarm.profile.getProfile(
        null, results.identity.owner, function(err, profile) {
