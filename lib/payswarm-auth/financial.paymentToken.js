...
api.removePaymentToken = function(actor, id, callback) {
  async.waterfall([
    ...
    function(record, callback) {
      ...
      // if expiration time is 0, remove immediately
      if(expires === 0) {
    ...
    function(expires, callback) {
      if(expires === 0) {
        return callback(null, null, expires);
      }
      payswarm.db.collections.paymentToken.findOne(
        {id: payswarm.db.hash(id)}, {}, function(err, record) {
          callback(err, record, expires);
        });
    },
    function(record, expires, callback) {
      if(expires === 0) {
        return callback(null, null);
      }

      var token = record.paymentToken;
      if(token.psaStatus !== 'deleted' && !token.psaVerified) {
        return callback(new PaySwarmError(
