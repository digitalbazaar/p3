  ...
  async.waterfall([
    ...
    function(callback) {
      ...
      payswarm.events.on(EVENT_SCAN, function(event) {
        ...
        var options = {};
        if(event && event.details && event.details.paymentTokenId) {
  ...
  async.waterfall([
    ...
    function(token, meta, callback) {
      ...
      for(var i = 0; i < 2; ++i) {
        ...
        var refenceId = 'payswarm.verifyPaymentToken.' +
    ...
    function(txns, callback) {
      ...
      payswarm.db.collections.paymentToken.update(
        ...
        {$set: {
          ...
          'meta.verify': {transactions: txns, ready: false}
        }},
...
function _runWorker(options, callback) {
  // get time that tokens are considered idle
  var now = +new Date();
  var expiration = payswarm.config.financial.paymentToken
  ...
  payswarm.logger.debug(
    'running payment token worker (' + workerId + ') to find unverified ' +
  ...
  async.until(function() {return done;}, function(loopCallback) {
    async.waterfall([
      ...
      function(record, callback) {
        if(record === null) {
          if(options.id) {
            ...
            return callback(new PaySwarmError(
              'PaymenToken not found.',
              MODULE_TYPE + '.PaymentTokenNotFound'));
          }
          // done, no unverified payment tokens remain
          done = true;
          return loopCallback();
        }

        // if not ready to verify, ensure withdrawals have settled
        if(record.meta.verify && !record.meta.verify.ready) {
          return _checkVerifyTransactions(record.meta, function(err, settled) {
            if(!err && settled) {
              record.meta.verify.ready = true;
            }
      ...
      function(record, callback) {
        ...
        if(record.meta.verify && record.meta.verify.ready) {
          update.$set['meta.verify.ready'] = true;
        }
        payswarm.db.collections.paymentToken.update(
          ...
          callback(null, record.paymentToken);
        });
      },
      function(token, callback) {
        // send event
    ...
    if(err) {
      payswarm.logger.error(
        'error while scanning payment tokens', {error: err});
    }
