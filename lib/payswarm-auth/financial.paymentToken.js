...
api.verifyPaymentToken = function(actor, id, options, callback) {
  if(!options.amounts) {
    return callback(new PaySwarmError(
      ...
      MODULE_TYPE + '.InvalidPaymentTokenVerifyParameters', {id: id}));
  }
  ...
  async.waterfall([
    ...
    function(token, meta, callback) {
      ...
      if(!meta.verify || !meta.verify.transactions ||
        meta.verify.transactions.length === 0) {
        // FIXME: send event that will trigger email for human intervention
        // to fix this corner-case problem
        return callback(new PaySwarmError(
          'Payment token verification data not found.',
          MODULE_TYPE + '.PaymentTokenVerifyDataNotFound', {id: id}));
      }
      ...
      async.forEach(meta.verify.transactions, function(txn, callback) {
        ...
        if(!err && !settled) {
          // FIXME: send event to trigger email in this case? maybe only
          // if a certain period of time has passed (set via config?)
