  ...
  async.waterfall([
    ...
    function setupCollections(callback) {
      ...
      payswarm.db.createIndexes([{
        ...
        collection: 'paymentToken',
        fields: {'meta.updated': -1, 'paymentToken.psaVerified': 1, id: 1},
        options: {unique: true, background: true}
      }, {
        // used by workers to find expired tokens
        collection: 'paymentToken',
        fields: {'paymentToken.psaExpires': 1, id: 1},
        options: {unique: true, background: true}
      }, {
        ...
        collection: 'paymentToken',
        fields: {'paymentToken.psaVerified': 1},
        options: {unique: false, background: true}
      }], callback);
    },
      ...
      async.forEachSeries(
        ...
        function(options, callback) {
          ...
          payswarm.db.collections.paymentToken.findOne(
            {id: payswarm.db.hash(options.token.id)}, {},
            function(err, record) {
              if(err || !record) {
                return create();
              }
...
api.createPaymentToken = function(actor, options, callback) {
  ...
  var isAuthority = (!actor || actor.id === payswarm.config.authority.profile);

  // if gateway not provided, use default for source type
  if(gateway === null) {
    var gateways = payswarm.config.financial.defaults.paymentGateways;
  ...
  if(gateway && gateway.authorityOnly &&
    ((actor && actor.id !== payswarm.config.authority.profile) ||
  ...
  async.waterfall([
    ...
    function(callback) {
      ...
      var limit = payswarm.config.financial.paymentToken
    ...
    function(callback) {
      ...
      payswarm.db.collections.paymentToken.find({
        'paymentToken.psaVerified': false
      }, {'paymentToken.psaVerified': true}).count(function(err, count) {
        if(err) {
          return callback(err);
        }
        if(count >= limit) {
    ...
    function(callback) {
      ...
      var tokenCfg = payswarm.config.financial.paymentToken;
      var max = payswarm.money.createOutgoingExternalMoney(
        tokenCfg.maxVerifyAmount);
      var maxTotal = max.multiply(tokenCfg.verifyAmounts);
      var verifySource = payswarm.config.financial.paymentTokenVerifyAccount;
      ...
      payswarm.financial.getAccount(null, verifySource,
        ...
        var balance = payswarm.money.createOutgoingExternalMoney(
          account.balance);
        if(balance.compareTo(maxTotal) < 0) {
          ...
          err = new PaySwarmError(
            ...
            MODULE_TYPE + '.PaymentTokenCreationError',
            {'public': true});
      ...
      api.createPaymentTokenVerifyData(actor, token.id, function(err) {
        if(err) {
          ...
          return async.waterfall([
            ...
            // server-side error
            if(!err &&
              cause.name === MODULE_TYPE + '.PaymentTokenCreationError') {
...
api.getIdentityPaymentTokens = function(actor, identityId, gateway, callback) {
  ...
  async.waterfall([
    ...
    function(records, callback) {
      ...
      var create = payswarm.config.financial.createDefaultPaymentTokens;
...
api.updatePaymentToken = function(actor, paymentTokenUpdate, callback) {
  async.auto({
    checkPermission: function(callback) {
      payswarm.profile.checkActorPermissionForObject(
        actor, {id: paymentTokenUpdate.id},
        PERMISSIONS.PTOKEN_ADMIN, PERMISSIONS.PTOKEN_EDIT,
        _checkPaymentTokenOwner, callback);
    },
    getPaymentToken: ['checkPermission', function(callback) {
      // Note: The payment token is used to determine the payment method
      // and which fields are allowed to be updated.
      payswarm.financial.getPaymentToken(actor, paymentTokenUpdate.id,
        function(err, paymentToken) {
          callback(err, paymentToken);
        });
    }],
    sanitizePaymentToken: ['getPaymentToken', function(callback, results) {
    ...
    update: ['sanitizePaymentToken', function(callback, results) {
      payswarm.db.collections.paymentToken.update(
        {id: payswarm.db.hash(results.sanitizePaymentToken.id)},
  ...
  async.auto({
    ...
    expireToken: ['getExpires', function(callback, results) {
      ...
      payswarm.db.collections.paymentToken.update({
        ...
        $or: [
          {'paymentToken.psaVerified': true},
          {'paymentToken.psaStatus': 'disabled'}
        ]
      }, {
        $set: {
          'meta.updated': now,
          'paymentToken.psaStatus': 'deleted',
          'paymentToken.psaExpires': now + expires
        }
    ...
    removeToken: ['getExpiredToken', function(callback, results) {
      ...
      var expires = results.getExpires;
      if(expires === 0) {
        return payswarm.db.collections.paymentToken.remove({
          id: payswarm.db.hash(id),
          'paymentToken.psaStatus': 'deleted'
        ...
        if(token.psaStatus !== 'deleted') {
          ...
          if(token.backupSourceFor && token.backupSourceFor.length > 0) {
            return callback(new PaySwarmError(
              ...
              MODULE_TYPE + '.BackupSourcePaymentToken',
              {id: id, httpStatusCode: 400, 'public': true}));
          }
        }
      }
    ...
    function(record, callback) {
      ...
      payswarm.db.collections.paymentToken.update(
        {id: payswarm.db.hash(id)}, {
          ...
          $unset: {'paymentToken.psaExpires': true}
        }, payswarm.db.writeOptions, function(err) {
          callback(err);
        });
    },
  ...
  async.waterfall([
    ...
    function(token, meta, callback) {
      ...
      for(var i = 0; i < amounts; ++i) {
        ...
        amount = payswarm.money.createOutgoingExternalMoney(amount);
      ...
      async.forEachSeries(txns, function(txn, callback) {
        ...
        if(err && err.name === 'payswarm.financial.InsufficientFunds') {
          ...
          payswarm.events.emit({
            type: 'common.PaymentToken.verifyBalanceTooLow',
            details: {
              account: payswarm.config.financial.paymentTokenVerifyAccount
            }
          });
        }
...
api.verifyPaymentToken = function(actor, id, options, callback) {
  ...
  async.waterfall([
    ...
    function(token, meta, callback) {
      ...
      if(token.psaVerified || options.force) {
      ...
      var max = payswarm.config.financial.paymentToken.maxVerifyAttempts;
      if(meta.verify.attempts >= max) {
        return callback(new PaySwarmError(
          'The maximum number of verification attempts has been exceeded.',
          MODULE_TYPE + '.MaxVerifyAttemptsExceeded', {
            id: id,
            'public': true
          }));
      }
      ...
      for(var i = 0; i < VS.length; ++i) {
        VM.push(payswarm.money.createOutgoingExternalMoney(VS[i].amount));
      }
      VM.sort(mcmp);
      ...
      for(var i = 0; i < AS.length; ++i) {
        AM.push(payswarm.money.createOutgoingExternalMoney(AS[i]));
      }
      AM.sort(mcmp);
    ...
    function(token, verified, callback) {
      ...
      if(!verified) {
        return payswarm.db.collections.paymentToken.update(
          {id: payswarm.db.hash(id)},
    ...
    if(!err) {
      if(!verified) {
        err = new PaySwarmError(
          'Verification failed.',
          MODULE_TYPE + '.VerificationFailed', {
            id: id,
            'public': true
          });
      }
  ...
  async.waterfall([
    ...
    function(callback) {
      if(status === 'voided') {
        ...
        return payswarm.db.collections.paymentToken.update(
          {id: payswarm.db.hash(tokenId), 'paymentToken.psaStatus': 'active'},
          {$set: {
            'meta.updated': +new Date(),
            'paymentToken.psaStatus': 'disabled'
          }}, payswarm.db.writeOptions, function(err) {
            if(!err) {
              err = new PaySwarmError(
                ...
                MODULE_TYPE + '.VerificationFailed', {'public': true});
            }
            callback(err);
          });
      }
...
function _runWorker(options, callback) {
  ...
  var cfg = payswarm.config.financial.paymentToken;
  if(options.algorithm === 'verify') {
    ...
    query['paymentToken.psaStatus'] = 'active';
  }
  else if(options.algorithm === 'remove') {
    query['paymentToken.psaExpires'] = {$lte: now};
    query['paymentToken.psaStatus'] = 'deleted';
  }
  else {
    return callback(new PaySwarmError(
  ...
  if(options.id) {
    ...
    var workerExpired =
      (+new Date() - payswarm.config.financial.paymentToken.worker.schedule);
    query.$or = [
      {workerId: null},
          ...
          if(record.meta.verify && !record.paymentToken.psaVerifyReady) {
            return _checkVerifyTransactions(
              ...
              if(!err && status === 'settled') {
                ...
                record.paymentToken.psaStatus = 'disabled';
              }
              callback(null, record, record.paymentToken.psaVerifyReady);
            });
          }
        }
      ...
      function(record, makeReady, callback) {
        if(options.algorithm === 'remove') {
          ...
          payswarm.db.collections.paymentToken.update(
            ...
            if(record.paymentToken.psaVerifyReady &&
              record.meta.verify.notified === now) {
              // include verify transactions if not in production mode
              var transactions = null;
              if(payswarm.config.environment !== 'production') {
