...
bedrock.events.on('bedrock.init', function(callback) {
  ...
  payswarm.events.on(EVENT_SCAN, function(event) {
    payswarm.logger.verbose('got payment token scan event', event);
    var options = {};
...
api.createPaymentToken = function(actor, options, callback) {
  ...
  var isAuthority = (!actor || actor.id === payswarm.config.authority.id);
  ...
  if(gateway && gateway.authorityOnly &&
    ((actor && actor.id !== payswarm.config.authority.id) ||
  ...
  async.until(function() {return done;}, function(loopCallback) {
    ...
    if(options.reschedule) {
      ...
      setTimeout(function() {
        ...
        payswarm.events.emitLater(event);
      }, options.reschedule);
    }
    if(callback) {
      callback(err);
