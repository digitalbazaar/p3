...
api.createPaymentToken = function(actor, options, callback) {
  ...
  // to use 'Authority' gateway, the actor must be null or authority and
  ...
  if(gateway === 'Authority' &&
    ...
    return callback(new PaySwarmError(
      'Could not create payment token. Permission denied.',
...
api.createPaymentTokenVerifyData = function(actor, id, callback) {
  async.waterfall([
    ...
    function(token, meta, callback) {
      ...
      var txns = [];
      for(var i = 0; i < 2; ++i) {
        // generate random amount under $0.50 that is > 0
