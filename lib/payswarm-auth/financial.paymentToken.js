...
api.createPaymentToken = function(actor, options, callback) {
  ...
  async.waterfall([
    ...
    function(callback) {
      ...
      var maxTotal = payswarm.money.createExternalMoney(amounts * max);
      ...
      payswarm.financial.getAccount(null, verifySource,
        ...
        var balance = payswarm.money.createExternalMoney(account.balance);
  ...
  async.waterfall([
    ...
    function(token, meta, callback) {
      ...
      for(var i = 0; i < VS.length; ++i) {
        VM.push(payswarm.money.createExternalMoney(VS[i].amount));
      }
      VM.sort(mcmp);
      ...
      for(var i = 0; i < AS.length; ++i) {
        AM.push(payswarm.money.createExternalMoney(AS[i]));
      }
      AM.sort(mcmp);
