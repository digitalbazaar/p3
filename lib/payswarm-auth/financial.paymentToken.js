  ...
  async.auto({
    ...
    expireToken: ['getExpires', function(callback, results) {
      ...
      payswarm.db.collections.paymentToken.update({
        ...
        'paymentToken.backupSourceFor.0': {$exists: true},
        $or: [
    ...
    getExpiredToken: ['expireToken', function(callback, results) {
      payswarm.db.collections.paymentToken.findOne(
        {id: payswarm.db.hash(id)}, {}, callback);
    }],
    removeToken: ['getExpiredToken', function(cb, results) {
    ...
    callback(err, results.getExpiredToken || null);
  });
};
