...
function _getTransactionIdentityAndAsset(transaction, callback) {
  async.waterfall([
    function(callback) {
      ...
      // use destination owner for withdrawal
      if(jsonld.hasValue(transaction, 'type', 'com:Withdrawal')) {
        if(!transaction.destination.owner) {
          return callback(new PaySwarmError(
            'Could not determine Withdrawal destination\'s identity; ' +
            'invalid Withdrawal format.', MODULE_TYPE + '.InvalidTransaction',
            {transaction: transaction}));
        }
        return callback(null, transaction.destination.owner);
      }
