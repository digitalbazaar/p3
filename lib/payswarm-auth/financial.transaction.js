...
function _authorizeTransaction(transaction, identities, callback) {
  ...
  var isDeposit = jsonld.hasValue(transaction, 'type', 'Deposit');
  if(isDeposit) {
    // if deposit was triggered by insufficient funds, then assume no
    // stored value will be accrued by its authorization (relies upon
    // contract code to enforce this)
    ...
    _checkDepositDestinations(transaction, function(err) {
      if(err) {
        return callback(err);
      }
  ...
  async.auto({
    ...
    updateTransaction: ['updateAccount', function(callback, results) {
      ...
      _markTransactionAuthorized(
        transaction, identities, {triggered: results.fundSource.triggered},
        callback);
    }]
  }, function(err) {
    callback(err);
...
function _checkDepositDestinations(transaction, callback) {
  ...
  async.each(Object.keys(amounts), function(dst, callback) {
...
function _markDepositDestination(transaction, dst, amount, callback) {
  ...
  async.waterfall([
    ...
    function(record, callback) {
      ...
      var pending = new Money(record.credit.pending).add(amount);
      var update = {
        $set: {
          updateId: updateId,
          'meta.updated': +new Date(),
...
function _clearDepositDestinations(transaction, callback) {
  // if deposit was triggered by insufficient funds, then there are
  // no destination entries to clean up
  if(transaction.triggerReason === 'InsufficientFunds') {
    return callback();
  }
  // get total transfer amount for each destination
...
function _clearDepositDestination(transaction, dst, amount, callback) {
  ...
  async.waterfall([
    ...
    function(record, cb) {
      ...
      var pending = new Money(record.credit.pending).subtract(amount);
      var update = {
        $set: {
          updateId: updateId,
          'meta.updated': +new Date(),
...
function _fundAccount(transaction, account, callback) {
  ...
  // determine if source has sufficient funds for purchase
  ...
  // a deposit is being associated with the purchase, so not using credit
  rval.startCreditPeriod = false;

  // authorize deposit to fund account w/purchase amount
  async.waterfall([
    function(callback) {
      var deposit = {
        ...
        payee: [{
          ...
          currency: 'USD',
          comment: 'Automated deposit to complete purchase: ' + transaction.id
        }],
...
function _processTransaction(transactionId, callback) {
  ...
  async.waterfall([
    ...
    function(record, callback) {
      ...
      // ensure contract authorized for triggered deposits
    ...
    function(record, callback) {
      ...
      if(record.credit && record.credit.incoming[transactionHash] === true) {
        update.$set['credit.pending'] = new Money(
          record.credit.pending).subtract(amount);
