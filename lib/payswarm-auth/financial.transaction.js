...
function _authorizeTransaction(transaction, identities, callback) {
  ...
  async.auto({
    ...
    checkSource: ['getSource', function(callback, results) {
      ...
      // ensure balance is at or above minimum if autopay feature not enabled
      if(balance.compareTo(minBalance) < 0 && !account.psaAutoPay) {
      ...
      callback(null, {balance: balance, wasPositive: wasPositive});
    }],
    updateAccount: ['checkSource', function(cb, results) {
      // skip deposit, handled externally
      if(isDeposit) {
        return cb();
      }
      ...
      var balance = results.checkSource.balance;
      var wasPositive = results.checkSource.wasPositive;
      var update = {
        $set: {
          updateId: updateId,
          'meta.updated': +new Date(),
    ...
    ensureUpdated: ['updateTransaction', function(callback, results) {
      if(results.updateTransaction === 0) {
        // fire event to void transaction
        var event = {
          type: EVENT_VOID,
          details: {transactionId: transaction.id}
