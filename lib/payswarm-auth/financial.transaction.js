    ...
    function(record, callback) {
      ...
      payswarm.profile.checkActorPermissionForObject(
        ...
        _checkTransactionAccess(rel), function(err) {
          callback(err, record, rel);
        });
    },
    function(record, rel, callback) {
      if(!rel.isAssetProvider && 'assetProvider' in record.transaction) {
        // hide asset provider address
        delete record.transaction.assetProvider.address;
      }
      if(!rel.isAssetAcquirer && 'assetAcquirer' in record.transaction) {
        // hide asset acquirer address
        delete record.transaction.assetAcquirer.address;
      }
 ...
 *
 * @return an transaction access checker callback.
 */
function _checkTransactionAccess(rel) {
  rel = rel || {};
  return function(actor, txn, callback) {
    return _checkTransactionAccessResult(actor, txn, rel, callback);
  };
};
 ...
 * @param rel the object to update with the actor's relationship(s) to the
 *          transaction.
 * @param callback(err, allowed) called once the check is complete with
 *          true if access is allowed.
 */
function _checkTransactionAccessResult(actor, transaction, rel, callback) {
  ...
  async.auto({
    ...
    checkAccess: ['isAssetAcquirer', 'isAssetProvider', 'isSourceOwner',
      'isDestinationOwner', function(callback) {
        var hasAccess = (rel.isAssetAcquirer || rel.isAssetProvider ||
          rel.isSourceOwner || rel.isDestinationOwner);
