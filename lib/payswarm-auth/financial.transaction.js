...
function _fundAccount(transaction, account, options, callback) {
  ...
  // options.deposit is given or (instant transfer feature is enabled, contract
  // amount is less than minimum instant transfer amount, and a account backup
  // source is available)
  ...
  var canInstantTransfer = (isContract &&
    (options.deposit || (account.psaAllowInstantTransfer &&
    amount.compareTo(minInstantTransfer) >= 0 &&
    sources.length > 0)));
