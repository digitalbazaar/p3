...
bedrock.events.on(EVENT_SETTLE, function(event) {
  logger.verbose('got settle event', event);
...
bedrock.events.on(EVENT_VOID, function(event) {
  logger.verbose('got void event', event);
...
api.voidTransaction = function(transactionId, options, callback) {
  logger.verbose('voiding transaction', transactionId);
    ...
    function(callback) {
      var update = {
        $set: {
          ...
          'transaction.voided': bedrock.util.w3cDate(),
    ...
    function(records, callback) {
      ...
      async.eachSeries(records, function(record, callback) {
        ...
        if(txn) {
          ...
          txn['@context'] = payswarm.constants.PAYSWARM_CONTEXT_V1_URL;
        } else {
        ...
        // do in parallel
        async.each(cleanedRecords, function(record, callback) {
          var txn = record.transaction;
      ...
      async.until(function() {return inserted;}, function(callback) {
        async.waterfall([
          ...
          function(callback) {
            ...
            var txn = bedrock.util.extend({}, transaction);
            txn['@context'] = payswarm.constants.PAYSWARM_CONTEXT_V1_URL;
            if(isContract) {
  ...
  async.waterfall([
    function(callback) {
      ...
      if(assetAcquirers.length > 0) {
        ...
        if(bedrock.util.isObject(assetAcquirer)) {
    ...
    function(identityId, callback) {
      ...
      if(assets.length > 0) {
        asset = assets[0];
        if(bedrock.util.isObject(asset)) {
          asset = asset.id;
        }
      } else {
  ...
  async.auto({
    ...
    updateAccount: ['fundSource', function(cb, results) {
      ...
      brDatabase.collections.account.update(
        ...
        update, brDatabase.writeOptions, function(err, result) {
          if(err) {
            return cb(err);
          }
    ...
    function(callback) {
      var update = {
        $set: {
          state: SETTLE_STATE.AUTHORIZED,
          'meta.updated': +new Date(),
          'transaction.authorized': bedrock.util.w3cDate()
        },
...
function _processTransaction(transactionId, callback) {
  logger.verbose('processing transaction', transactionId);
  var transactionHash = brDatabase.hash(transactionId);
  async.waterfall([
    // get transaction
    function(callback) {
    ...
    function(callback) {
      ...
      brDatabase.collections.transaction.findAndModify(
        ...
        [['id', 'asc']],
        update,
        bedrock.util.extend({}, brDatabase.writeOptions, {
          upsert: false,
          'new': true,
...
function _addIncomingEntries(transaction, settleId, callback) {
  ...
  async.waterfall([
    function(callback) {
      ...
      async.eachSeries(Object.keys(accounts), function(dst, callback) {
        async.waterfall([
          function(callback) {
...
function _settleTransaction(transaction, settleId, callback) {
  ...
  async.waterfall([
    ...
    function(callback) {
      var now = new Date();
      var settled = bedrock.util.w3cDate(now);
      brDatabase.collections.transaction.update(
        ...
        brDatabase.writeOptions, function(err, result) {
          callback(err, settled, result);
        });
    },
    // if no update, get FT's state.
    function(settled, result, callback) {
      if(result.result.n === 0) {
        return brDatabase.collections.transaction.findOne(
          {id: transactionHash}, {
            state: true,
  ...
  async.until(function() {return done;}, function(callback) {
    async.waterfall([
      ...
      function(result, callback) {
        ...
        if(result.result.n === 1 || options.id) {
    ...
    logger.verbose('transaction worker (' + workerId + ') finished.');
