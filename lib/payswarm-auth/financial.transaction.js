...
function _checkExternalTransactionStatus(transaction, callback) {
  ...
  var isBankAccount = jsonld.hasValue(account, 'type', 'bank:BankAccount');
  if(!isBankAccount) {
  ...
  if(transaction.psaStatusChecks >=
    payswarm.config.transaction.maxStatusChecks) {
    ...
    payswarm.logger.warning(err.message);
    return callback(err);
  }
...
function _addIncomingEntries(transaction, settleId, callback) {
  ...
  // get unique destination accounts (skip for withdrawals as destinations
  // are offsite)
  var transfers = jsonld.getValues(transaction, 'transfer');
  var accounts = {};
  transfers.forEach(function(transfer) {
  ...
  async.waterfall([
    function(callback) {
      // skip withdrawal, handled externally
      if(isWithdrawal) {
        return callback();
      }
