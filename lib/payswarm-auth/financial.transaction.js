    ...
    function(records, callback) {
      ...
      async.forEachSeries(records, function(record, callback) {
        ...
        async.waterfall([
          ...
          function(callback) {
            if(!rel.isAssetProvider && 'assetProvider' in txn) {
              // hide asset provider address
              delete txn.assetProvider.address;
            }
...
function _checkExternalTransactionStatus(transaction, callback) {
  ...
  async.waterfall([
    ...
    function(result, callback) {
      ...
      // FIXME: emit error that triggers email for human intervention
      ...
      callback(new PaySwarmError(
        ...
        {transaction: transaction.id, result: result}));
    }
  ], callback);
}
