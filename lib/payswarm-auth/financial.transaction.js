    ...
    function(callback) {
      ...
      payswarm.db.collections.account.findOne(
        {id: payswarm.db.hash(src)}, {
          ...
          'account.psaAllowInstantTransfer': true,
          'account.psaMinInstantTransfer': true
        }, function(err, record) {
          if(err) {
            return callback(err);
          }
          ...
          // ensure new balance would be at or above minimum, but also do
          // instant transfer check
          if(balance.compareTo(minBalance) < 0) {
            // can't do instant transfer if transaction is not a contract,
            // instant transfer feature not enabled, contract amount is less
            // than minimum instant transfer amount, or no account backup
            // sources available
            var minInstantTransfer = new Money(
              account.psaMinInstantTransfer || 0);
            var isContract = jsonld.hasValue(transaction, 'type', 'Contract');
            var sources = jsonld.getValues(account, 'backupSource');
            if(!isContract || !account.psaAllowInstantTransfer ||
...
function _fundAccount(transaction, account, callback) {
  ...
  // can't do instant transfer if transaction is not a contract, instant
  // transfer feature not enabled, contract amount is less than minimum
  // instant transfer amount, or no account backup sources available
  var minInstantTransfer = new Money(account.psaMinInstantTransfer || 0);
  var isContract = jsonld.hasValue(transaction, 'type', 'Contract');
  var sources = jsonld.getValues(account, 'backupSource').slice();
  if(!isContract || !account.psaAllowInstantTransfer ||
    amount.compareTo(minInstantTransfer) < 0 || sources.length === 0) {
    return callback(new PaySwarmError(
      'Insufficient funds in the source FinancialAccount.',
      MODULE_TYPE + '.InsufficientFunds', {
        httpStatusCode: 400,
      ...
      payswarm.db.collections.transaction.findOne(
        ...
        function(err, trigger) {
          ...
          if(trigger.state === SETTLE_STATE.PENDING) {
            ...
            var inc = {};
            inc['transaction.psaSettleAfter'] =
              payswarm.config.financial.transaction.triggerSettleAfterIncrement;
 ...
 * payment gateway may determine that the Deposit is automatically considered
 * settleable. It may, alternatively, perform a credit card capture on any
 * held funds before determining that the Deposit is setteable. If the payment
 * source or destination of the Transaction is a bank account, an inquiry into
 * a pending ACH transaction's state may need to be made.
 *
    ...
    function(result, callback) {
      ...
      if(!result || result.status === 'pending' || result.status === 'error') {
        ...
        if(result && 'settleAfterIncrement' in result) {
          inc['transaction.psaSettleAfter'] = result.settleAfterIncrement;
        }
      ...
      // update any psaGateway flags
      var set = {'meta.updated': +new Date()};
      Object.keys(transaction).forEach(function(key) {
        if(key.indexOf('psaGateway') === 0) {
          set['transaction.' + key] = transaction[key];
        }
      });
