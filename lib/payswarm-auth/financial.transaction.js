  ...
  async.waterfall([
    ...
    function(callback) {
      ...
      async.forEachSeries(Object.keys(accounts), function(dst, callback) {
        // check for existence
        payswarm.db.collections.account.findOne(
          {id: payswarm.db.hash(dst)}, {owner: true},
          function(err, record) {
            if(!err && !record) {
      ...
      async.until(function() {return inserted;}, function(callback) {
        async.waterfall([
          function(callback) {
            ...
            payswarm.db.collections.transaction.find(
              {identity: identityIdHash, asset: assetIdHash},
          ...
          function(callback) {
            ...
            var record = {
              ...
              asset: assetIdHash,
              // identity is *only* the identity of the source account owner
              identity: identityIdHash,
