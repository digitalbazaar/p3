  ...
  if(triggered) {
    ...
    if(!isTriggeredDeposit) {
      return callback(new PaySwarmError(
        ...
        MODULE_TYPE + '.InvalidTriggeredTransactionType', {
          httpStatusCode: 400,
          'public': true,
          transactionId: transaction.id
        }));
    ...
    if(triggered.triggeredBy !== transaction.id) {
      return callback(new PaySwarmError(
        ...
        'Contract.',
        MODULE_TYPE + '.InvalidTriggeredTransaction', {
          httpStatusCode: 400,
          'public': true,
          transactionId: transaction.id
        }));
  ...
  async.waterfall([
    ...
    if(err) {
      ...
      if(payswarm.db.isDuplicateError(err)) {
        ...
        else if(!(err instanceof PaySwarmError)) {
          err = new PaySwarmError(
            'Transaction could not be authorized.',
            MODULE_TYPE + '.TransactionAuthorizationError', {
              transactionId: transaction.id, 'public': true
            }, err);
        }
      }
