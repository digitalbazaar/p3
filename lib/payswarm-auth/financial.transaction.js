...
function _authorizeTransaction(transaction, identities, callback) {
  ...
  async.auto({
    ...
    getNewBalance: ['getSource', function(callback, results) {
    ...
    fundAccount: ['getNewBalance', function(callback, results) {
      // determine if source has sufficient funds already
      var balance = results.getNewBalance.balance;
      var minBalance = results.getNewBalance.minBalance;
      var sufficientFunds = (balance.compareTo(minBalance) >= 0);

      // raise error if insufficient funds and autopay feature not enabled
      var account = results.getSource.account;
      if(!sufficientFunds && !account.psaAutoPay) {
      ...
      // fund account
      // FIXME: deposit more funds into account
      //_fundAccountForPurchase(callback);
      callback(null, null);
    }],
    updateAccount: ['fundAccount', function(cb, results) {
      // get next update ID
      var record = results.getSource;
      ...
      var balance = results.getNewBalance.balance;
      var wasPositive = results.getNewBalance.wasPositive;
      var update = {
        $set: {
          updateId: updateId,
          'meta.updated': +new Date(),
