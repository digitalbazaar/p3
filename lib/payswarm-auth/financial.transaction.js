      ...
      payswarm.db.collections.account.findOne(
        {id: payswarm.db.hash(src)}, {
          ...
          if('creditLimit' in record.account &&
            !record.account.psaCreditDisabled) {
            minBalance = new Money(record.account.creditLimit).setNegative();
          }
          else {
            minBalance = new Money(0);
          }

          // ensure new balance would be at or above minimum
      ...
      async.forEachSeries(records, function(record, callback) {
        ...
        async.waterfall([
          ...
          function(callback) {
            ...
            if(flags.external) {
              delete txn.psaExternalStatusChecks;
              delete txn.psaGatewayApprovalCode;
              delete txn.psaGatewayRefId;

              if('destination' in txn) {
                delete txn.destination.paymentToken;
                delete txn.destination.psaStatus;
                delete txn.destination.psaVerified;
                delete txn.destination.psaVerifyReady;
              }

              if('source' in txn) {
                delete txn.source.paymentToken;
                delete txn.source.psaStatus;
                delete txn.source.psaVerified;
                delete txn.source.psaVerifyReady;
              }

              if('vendor' in txn && 'address' in txn.vendor) {
                delete txn.vendor.address.psaValidated;
              }

              if('assetAcquirer' in txn && 'address' in txn.assetAcquirer) {
                delete txn.assetAcquirer.address.psaValidated;
              }
            }
            callback();
          }
...
function _authorizeTransaction(transaction, identities, callback) {
  ...
  async.auto({
    ...
    fundSource: ['getSource', function(callback, results) {
      ...
      _fundAccount(transaction, record.account, callback);
    }],
    updateAccount: ['fundSource', function(cb, results) {
      ...
      var startCreditPeriod = results.fundSource.startCreditPeriod;
      var update = {
        $set: {
          updateId: updateId,
          'meta.updated': +new Date(),
      ...
      // indicate credit use has started
      if(startCreditPeriod) {
        update.$set['account.creditUseStarted'] = transaction.created;
        update.$set.creditUseStarted = new Date(transaction.created);
      }
      // clear credit usage date
      else {
        update.$unset = {
          'account.creditUseStarted': true,
          creditUseStarted: true
        };
      }
      var transactionHash = payswarm.db.hash(transaction.id);
      update.$set['outgoing.' + transactionHash] = transaction.psaSettleAfter;
        ...
        update, payswarm.db.writeOptions, function(err, n) {
          ...
          if(n === 0) {
            ...
            return process.nextTick(_authorizeTransaction.bind(
              null, transaction, identities, callback));
          }
          cb();
        });
    }],
    updateTransaction: ['updateAccount', function(callback, results) {
      ...
      _markTransactionAuthorized(
        transaction, identities, {
          usedCredit: results.fundSource.usedCredit,
...
function _markDepositDestination(transaction, dst, amount, callback) {
  var incoming = 'credit.incoming.' + payswarm.db.hash(transaction.id);
  var dstHash = payswarm.db.hash(dst);
  async.waterfall([
    function(callback) {
    ...
    function(record, cb) {
      ...
      if(record.account.psaAllowStoredValue ||
      ...
      if(!(record.credit && 'snapshot' in record.credit)) {
        ...
        return cb(new PaySwarmError(
          ...
          MODULE_TYPE + '.InvalidDestination',
      ...
      if(balance.compareTo(Money.ZERO) > 0) {
        return cb(new PaySwarmError(
          ...
          MODULE_TYPE + '.AccruedStoredValue',
          {transaction: transaction, destination: dst}));
      }
      cb(null, record);
    },
...
function _clearDepositDestination(transaction, dst, amount, callback) {
  ...
  async.waterfall([
    function(callback) {
      ...
      payswarm.db.collections.account.findOne(
        query, {updateId: true, 'credit': true}, callback);
    },
    function(record, cb) {
      // record already updated, skip
      if(!record) {
      ...
      // update FA w/new credit.pending and remove credit.incoming entry
      ...
      payswarm.db.collections.account.update(
        {id: dstHash, updateId: record.updateId},
        update, payswarm.db.writeOptions, cb);
    },
    function(n, info, cb) {
      // if account not updated, try again
      if(n === 0) {
 ...
 */
function _fundAccount(transaction, account, callback) {
  var rval = {};
  ...
  rval.startCreditPeriod = (!balance.isNegative() && rval.balance.isNegative());
  rval.usedCredit = rval.balance.isNegative();

  // determine minimum allowed balance (use credit limit if available)
  var minBalance;
  if('creditLimit' in account && !account.psaCreditDisabled) {
    minBalance = new Money(account.creditLimit).setNegative();
  }
  else {
    minBalance = new Money(0);
  }
  ...
  // account has sufficient funds, return early
  if(sufficientFunds) {
    return callback(null, rval);
  }

  // can't fund if transaction is not a contract, autopay feature not enabled,
  // or contract amount is less than the minimum autopay amount
  var minAutoPay = new Money(account.psaMinAutoPay || 0);
  var isContract = jsonld.hasValue(transaction, 'type', 'Contract');
  if(!isContract || !account.psaAutoPay || amount.compareTo(minAutoPay) < 0) {
    return callback(new PaySwarmError(
      'Insufficient funds in the source FinancialAccount.',
      MODULE_TYPE + '.InsufficientFunds', {
        httpStatusCode: 400,
  ...
  rval.startCreditPeriod = false;

  // authorize deposit to fund account w/contract amount
  async.waterfall([
    function(callback) {
      var deposit = {
        ...
        payee: [{
          ...
          currency: 'USD',
          comment: 'Automated deposit to complete contract: ' + transaction.id
        }],
        source: account.backupSource,
 ...
 *          [usedCredit] true if the Transaction resulted in the use of credit.
 *          [accountRecord] if usedCredit is true, then this option is also
 *            given and it provides minimal information from the record
...
function _markTransactionAuthorized(
  ...
  async.waterfall([
    function(callback) {
      ...
      // if credit was used and source account's credit line is not backed,
      // then include payoff count w/transaction
      if(options.usedCredit) {
        var record = options.accountRecord;
        var source = record.account;
        if(!source.psaCreditBacked) {
          var payoffs = record.credit ? record.credit.payoffs || '0' : '0';
          update.$set['transaction.psaCreditPayoffs'] = payoffs;
        }
      }
      ...
      payswarm.db.collections.transaction.update(
        {id: payswarm.db.hash(transaction.id), state: SETTLE_STATE.PENDING},
        update, payswarm.db.writeOptions, callback);
    },
    ...
    function(n, info, callback) {
      if(n === 0) {
        return callback(new PaySwarmError(
          ...
          MODULE_TYPE + '.AuthorizationError'));
      }
      callback();
    }
...
function _processTransaction(transactionId, callback) {
  ...
  async.waterfall([
    ...
    // check contracts with psaCreditPayoffs property
    function(record, callback) {
      var transaction = record.transaction;
      var isContract = jsonld.hasValue(transaction, 'type', 'Contract');
      if(!isContract || !('psaCreditPayoffs' in transaction)) {
      ...
      async.waterfall([
        function(callback) {
          ...
          payswarm.db.collections.account.findOne(
            {id: payswarm.db.hash(src)}, {'credit.payoffs': true},
            callback);
        },
        function(accountRecord, callback) {
          ...
          var prevPayoffs = new BigNumber(transaction.psaCreditPayoffs);
          ...
          inc['transaction.psaSettleAfter'] =
            payswarm.config.financial.transaction
          ...
          payswarm.db.collections.transaction.update(
            ...
            payswarm.db.writeOptions, function(err) {
              ...
              callback(new PaySwarmError(
                ...
                'back yet.',
                MODULE_TYPE + '.CreditPayoffPending',
    ...
    function(record, callback) {
      ...
      payswarm.db.collections.transaction.findOne(
        {id: payswarm.db.hash(transaction.triggeredBy)}, {state: true},
        function(err, trigger) {
          ...
          if(!trigger) {
            ...
            return callback(new PaySwarmError(
              ...
              MODULE_TYPE + '.TriggerTransactionNotFound', {
                transactionId: transaction.id,
          ...
          if(trigger.state === SETTLE_STATE.PENDING) {
            return callback(new PaySwarmError(
              'The Transaction is still pending because it was triggered by ' +
              'another one that is pending.',
              MODULE_TYPE + '.TriggerTransactionPending', {
                transactionId: transaction.id,
                triggeredBy: transaction.triggeredBy
              }));
          }
          if(trigger.state === SETTLE_STATE.VOIDING ||
            ...
            return callback(new PaySwarmError(
              ...
              MODULE_TYPE + '.TriggerTransactionVoided', {
                transactionId: transaction.id,
                triggeredBy: transaction.triggeredBy
              }));
          }
...
function _settleDestinationAccount(
  ...
  async.waterfall([
    function(callback) {
      ...
      payswarm.db.collections.account.findOne(query, fields, callback);
    },
    function(record, callback) {
      if(!record) {
      ...
      var balance = new Money(record.account.balance);
      var wasNegative = balance.isNegative();
      balance = balance.add(amount);
      ...
      // increment credit payoff counter if negative balance paid off
      if(wasNegative && !balance.isNegative()) {
        var payoffs = record.credit ? record.credit.payoffs || '0' : '0';
