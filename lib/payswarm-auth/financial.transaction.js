  ...
  async.waterfall([
    ...
    function(callback) {
      ...
      payswarm.db.createIndexes([{
        ...
        // identity+asset+referenceId
        collection: 'transaction',
        fields: {identity: 1, asset: 1, referenceId: 1},
        options: {unique: true, background: true}
      }, {
        // asset+identity+id
        collection: 'transaction',
        fields: {asset: 1, identity: 1, id: 1},
        options: {unique: true, background: true}
      }, {
        // asset+date+id
        collection: 'transaction',
        ...
        // identity+date+id
        collection: 'transaction',
        fields: {identity: 1, created: -1, id: 1},
        options: {unique: true, background: true}
      }, {
        // source+date+id
        collection: 'transaction',
...
api.authorizeTransaction = function(transaction, duplicateQuery, callback) {
  ...
  async.waterfall([
    ...
    function(callback) {
      ...
      // skip lookup on external account
      if(src === 'urn:payswarm-external-account') {
        return callback();
      }
...
api.getTransaction = function(actor, id, callback) {
  async.waterfall([
    ...
    function(record, callback) {
      // check to make sure the caller is allowed to access the transaction
      payswarm.profile.checkActorPermissionForObject(
        ...
        _checkTransactionAccess, function(err) {
          callback(err, record);
        });
    },
    function(record, callback) {
...
function _processTransaction(transactionId, callback) {
  ...
  async.waterfall([
    ...
    function(record, callback) {
      ...
      var isDeposit = jsonld.hasValue(transaction, 'type', 'com:Deposit');
      var isWithdrawal = jsonld.hasValue(transaction, 'type', 'com:Withdrawal');
...
/**
 * Determines if the given transaction should be accessible to the provided
 * actor.
 *
 * @param actor the actor attempting to access the transaction.
 * @param transaction the transaction being accessed.
 * @param callback(err, allowed) called once the check is complete with
 *          true if access is allowed.
 */
function _checkTransactionAccess(actor, transaction, callback) {
  ...
  async.auto({
    ...
    getIdentities: function(callback) {
      payswarm.identity.getProfileIdentities(actor, actor.id, callback);
    },
    isAssetAcquirer: ['getIdentities', function(callback, results) {
      var isAssetAcquirer = false;
      var records = results.getIdentities;

      // check to see if any of the identities is the asset acquirer
      if(transaction.assetAcquirer) {
        var acquirer = transaction.assetAcquirer.id;
        isAssetAcquirer = _idMatchExists(acquirer, records, 'identity');
      }
      callback(null, isAssetAcquirer);
    }],
    isAssetProvider: ['getIdentities', function(callback, results) {
      var isAssetProvider = false;
      var records = results.getIdentities;

      // check to see if any of the identities is the asset provider
      if(transaction.assetProvider) {
        var provider = transaction.assetProvider.id;
        isAssetProvider = _idMatchExists(provider, records, 'identity');
      }
      callback(null, isAssetProvider);
    }],
    isSourceOwner: ['getIdentities', function(callback, results) {
      var isSourceOwner = false;
      var records = results.getIdentities;

      // check to see if any of the identities is the source owner
      if(transaction.source && transaction.source.owner) {
        var sourceOwner = transaction.source.owner;
        isSourceOwner = _idMatchExists(sourceOwner, records, 'identity');
      }
      callback(null, isSourceOwner);
    }],
    isDestinationOwner: ['getIdentities', function(callback, results) {
      var isDestinationOwner = false;
      var records = results.getIdentities;

      // check to see if any of the identities is the destination owner
      if(transaction.destination && transaction.destination.owner) {
        var destinationOwner = transaction.destination.owner;
        isDestinationOwner = _idMatchExists(
          destinationOwner, records, 'identity');
      }
      callback(null, isDestinationOwner);
    }],
    checkAccess: ['isAssetAcquirer', 'isAssetProvider', 'isSourceOwner',
      'isDestinationOwner', function(callback, results) {
        var hasAccess = (results.isAssetAcquirer || results.isAssetProvider ||
          results.isSourceOwner || results.isDestinationOwner);
        callback(null, hasAccess);
      }
    ],
  }, function(err, results) {
    callback(err, results ? results.checkAccess : null);
  });
}
...
function _checkExternalTransactionStatus(transaction, callback) {
  ...
  var isBankAccount = (
    jsonld.hasValue(account, 'type', 'bank:BankAccount') ||
    jsonld.hasValue(account, 'paymentMethod', 'bank:BankAccount'));
  if(!isBankAccount) {
  ...
  async.waterfall([
    function(callback) {
      ...
      if(transaction.psaStatusChecks >=
        payswarm.config.financial.transaction.maxStatusChecks) {
        // FIXME: ensure an event is fired that sends an email
        var err = new PaySwarmError(
          'Maximum number of status checks exceeded for transaction "' +
          transaction.id + '", intervention required to reset "psaStatusChecks" ' +
          'once the transaction has been settled via the payment gateway.',
      ...
      gateway.getTransactionStatus(transaction, callback);
    },
    function(result, callback) {
      var inc = {};
      if(result) {
        // increment number of status checks on txn
        inc['transaction.psaStatusChecks'] = 1;
      }

      // no result or external txn still pending
...
function _settleTransaction(transaction, settleId, callback) {
  ...
  transaction.transfer.forEach(function(transfer) {
    ...
    if(dst !== 'urn:payswarm-external-account') {
        ...
        payswarm.db.collections.transaction.update(
          ...
          payswarm.db.writeOptions, function(err) {
            ...
            if(err && !options.id && !payswarm.db.isDatabaseError(err)) {
              // FIXME: emit error event/log error?
              err = null;
            }
            callback(err);
          });
