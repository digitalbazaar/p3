...
function _authorizeTransaction(transaction, identities, callback) {
  ...
  async.auto({
    ...
    updateAccount: ['fundSource', function(cb, results) {
      ...
      var wasPositive = results.fundSource.wasPositive;
      var update = {
        $set: {
          updateId: updateId,
          'meta.updated': +new Date(),
      ...
      // indicate time credit was first used
      if(balance.isNegative() && wasPositive) {
        update.$set['account.creditUsed'] = transaction.created;
        update.$set.creditUsed = new Date(transaction.created);
      }
      // clear credit usage date
      else if(!balance.isNegative()) {
        update.$unset = {
          'account.creditUsed': true,
          creditUsed: true
        };
      }
    ...
    updateTransaction: ['updateAccount', function(callback) {
...
function _fundAccount(transaction, account, callback) {
  ...
  var amount = new Money(transaction.amount);
  var wasPositive = !balance.isNegative();
  balance = balance.subtract(amount);
  ...
  var sufficientFunds = (balance.compareTo(minBalance) >= 0);

  // account has sufficient funds, return early
  if(sufficientFunds) {
    return callback(null, {balance: balance, wasPositive: wasPositive});
  }

  // can't fund if transaction is not a contract or autopay feature not enabled
  var isContract = jsonld.hasValue(transaction, 'type', 'Contract');
  if(!isContract || !account.psaAutoPay) {
  ...
  // authorize deposit to fund account
  async.waterfall([
    function(callback) {
      var deposit = {
    ...
    /*function(deposit, callback) {
      // associate deposit with transaction
      transaction.relatedDeposit = deposit.id;
    }*/
 ...
 * @param callback(err) called once the operation completes.
 */
function _markTransactionAuthorized(transaction, identities, callback) {
  async.waterfall([
    function(callback) {
      payswarm.db.collections.transaction.update(
