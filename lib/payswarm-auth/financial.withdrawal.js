...
api.signWithdrawal = function(actor, withdrawal, options, callback) {
  ...
  async.auto({
    getSource: function(callback) {
      ...
      payswarm.financial.getAccount(actor, withdrawal.source, callback);
    },
    ...
    checkLimits: ['getSource', 'checkDestination', function(callback, results) {
      // get payee total and check against limits
      var amount = payswarm.money.createExternalMoney(0);
      var payees = jsonld.getValues(withdrawal, 'payee');
      for(var i in payees) {
        var payee = payees[i];
        // use external money amount, update total
        amount = amount.add(
          payswarm.money.createExternalMoney(payee.payeeRate));
      }
