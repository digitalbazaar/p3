...
api.signWithdrawal = function(actor, withdrawal, options, callback) {
  ...
  async.auto({
    ...
    getDestination: function(callback) {
      ...
      // force payee into withdrawal group, remove authority group
      jsonld.addValue(payees[0], 'payeeGroup', 'deposit');
      jsonld.removeValue(payees[0], 'payeeGroup', 'authority');
    ...
    createTransfers: ['getSource', 'addGatewayPayees', 'getId',
      function(callback, results) {
        ...
        var payees = jsonld.getValues(withdrwal, 'payee');
