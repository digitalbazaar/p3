...
api.signWithdrawal = function(actor, withdrawal, options, callback) {
  ...
  async.auto({
    ...
    getDestination: function(callback) {
      ...
      if(payees.length !== 1 ||
        ...
        return callback(new BedrockError(
          ...
          'financial account Payee.', MODULE_NS + '.InvalidWithdrawal'));
      }
    ...
    sign: ['adjustPrecision', function(callback) {
      ...
      // assign date
      withdrawal.created = bedrock.util.w3cDate();
      ...
      payswarm.financial.signTransaction(withdrawal, function(err, signed) {
        if(err) {
          return callback(new BedrockError(
            ...
            'this error.', MODULE_NS + '.WithdrawalSignatureError',
            {critical: true}, err));
        }
...
api.processWithdrawal = function(actor, withdrawal, options, callback) {
  ...
  if((now - parsed) > bedrock.config.financial.withdrawal.expiration) {
    return callback(new BedrockError(
  ...
  function _emitEvent(emitOptions) {
    ...
    if(emitOptions.details) {
      bedrock.util.extend(true, event.details, emitOptions.details);
    }
  ...
  async.auto({
    ...
    verify: ['getPublicKey', function(callback, results) {
      ...
      if(key.owner !== bedrock.config.authority.id) {
        return callback(new BedrockError(
          'Could not process Withdrawal; it was not signed by the ' +
          'PaySwarm Authority.',
          MODULE_NS + '.InvalidWithdrawalSigner'));
      }
      ...
      payswarm.security.verifyJsonLd(withdrawal, key, function(err, verified) {
        if(!verified) {
          return callback(err || new BedrockError(
            ...
            MODULE_NS + '.InvalidWithdrawalSignature'));
        }
    ...
    getIdentity: ['verify', function(callback) {
      ...
      brIdentity.getIdentity(null, identityId, function(err, identity) {
          ...
          return gateway.blindWithdrawal(withdrawal, function(blindErr, blind) {
            ...
            if(err.name === 'payswarm.paymentGateway.Declined') {
              payswarm.financial.voidTransaction(withdrawal.id, {
                ...
                if(err) {
                  logger.error('Error voiding declined withdrawal.', {
                    withdrawal: blind,
                    error: err
                  });
                }
            ...
            err = new BedrockError(
              ...
              MODULE_NS + '.WithdrawalNotApproved',
              {'public': true,
    ...
    updateTransaction: ['gatewayWithdrawalNotify', function(callback) {
      ...
      brDatabase.collections.transaction.update(
        {id: brDatabase.hash(withdrawal.id)},
        update, brDatabase.writeOptions, function(err) {
          callback(err);
        });
    }],
    notify: ['getIdentity', 'updateTransaction', function(callback, results) {
