...
api.signWithdrawal = function(actor, withdrawal, options, callback) {
  ...
  async.auto({
    ...
    checkDestination: ['getDestination', function(callback, results) {
      ...
      if(!options.verify &&
        jsonld.hasValue(dst, 'type', 'PaymentToken') && !dst.psaVerified) {
        return callback(new PaySwarmError(
    ...
    sign: ['createTransfers', function(callback, results) {
      ...
      var original = payswarm.money.createOutgoingExternalMoney(
        withdrawal.amount.toString());
      var total = payswarm.money.createOutgoingExternalMoney(0);
      ...
      for(var i in transfers) {
        ...
        if(transfer.destination === withdrawal.destination.id) {
          amount = payswarm.money.createOutgoingExternalMoney(
            transfer.amount);
        }
        else {
          amount = payswarm.money.createIncomingExternalMoney(
            transfer.amount);
        }
      ...
      // (any rounding affects external account)
...
api.processWithdrawal = function(actor, withdrawal, options, callback) {
  ...
  async.auto({
    ...
    authorize: ['verify', function(callback) {
      // set temporary psaSettleAfter to 1 day from now, to be
      // overwritten below by the payment gateway
      withdrawal.psaSettleAfter = (+new Date() + 1000*60*60*24);
