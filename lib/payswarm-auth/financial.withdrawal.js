...
api.processWithdrawal = function(actor, withdrawal, options, callback) {
  ...
  async.auto({
      ...
      gateway.creditWithdrawalDestination(withdrawal, function(err, result) {
        if(err) {
          ...
          err = new PaySwarmError(
            ...
            {'public': true, httpStatusCode: httpStatusCode, id: withdrawal.id},
            err);

          // blind the incomplete withdrawal for the event
          gateway.blindWithdrawal(withdrawal, function(err, blind) {
            // fake the async 'blind' result
            results.blind = blind;
    ...
    updateTransaction: ['gatewayWithdrawalNotify', function(callback, results) {
