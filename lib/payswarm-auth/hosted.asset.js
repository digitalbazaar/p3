...
/**
 * Sets the public key used to protect the content for a particular asset's
 * content. This will overwrite any existing public key, which will disable
 * access to any content hosted in the same base directory as the content URL
 * until the new associated private key is put into place.
 *
 * @param actor the Profile performing the action.
 * @param contentUrl the Asset content URL.
 * @param publicKey the PublicKey to associate with the content URL.
 * @param callback(err) called once the operation completes.
 */
api.setAssetContentPublicKey = function(
  actor, contentUrl, publicKey, callback) {
...
function _updateAsset(record, callback) {
  ...
  var isSigned = jsonld.hasValue(asset, 'signature');
