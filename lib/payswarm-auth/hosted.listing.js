...
function _getKeywords(asset) {
  ...
  var creators = jsonld.getValues(creator, 'creator');

  // add keywords from title
  titles.forEach(function(title) {
    keywords.apply.push(keywords, title.split(/\s/));
  });

  // add keywords from every property of creator
  creators.forEach(function(creator) {
    if(_.isString(creator)) {
      keywords.apply.push(keywords, creator.split(/\s/));
    }
    else if(_.isObject(creator)) {
      _.each(creator, function(value, key) {
        if(_.isString(value)) {
          keywords.apply.push(keywords, value.split(/\s/));
        }
      });
    }
  });
