  ...
  async.waterfall([
    ...
    function(callback) {
      ...
      payswarm.resource.asset.get(query, function(err, records) {
        ...
        // FIXME: should this step be necessary? fix resource so that
        // just the context URL is returned?
        asset['@context'] = payswarm.tools.getDefaultJsonLdContextUrl();
        callback(null, asset);
      });
    },
...
api.updateListing = function(actor, listing, asset, callback) {
  ...
  async.waterfall([
    ...
    function(listing, asset, callback) {
      ...
      if(asset) {
        listing = payswarm.tools.extend(true, {}, listing, {asset: asset});
      }
...
function _refreshListing(listing, options, callback) {
  ...
  async.auto({
    ...
    callback(null, results.getAsset[0], results.getAsset[1]);
  });
}
