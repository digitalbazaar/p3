...
api.createListing = function(actor, listing, callback) {
  async.waterfall([
    ...
    function(callback) {
      // FIXME: can't get asset if not yet signed, etc. ... which
      // is problematic for unpublished assets (need to be able to
      // get the unpublished asset if listing is not yet published
      // and a published asset is not available)
...
api.updateListing = function(actor, listing, asset, callback) {
  ...
  async.waterfall([
    ...
    function(listing, asset, callback) {
      ...
      listing = api.extend(true, {}, listing, {asset: asset});
...
function _updateListing(record, callback) {
  async.waterfall([
    ...
    function(refreshed, asset, callback) {
      ...
      var now = +new Date();
      api.updateListing(null, refreshed, asset, function(err) {
        if(err) {
          return callback(err);
        }
        record.listing = refreshed;
        if(record.meta) {
...
function _refreshListing(listing, options, callback) {
  ...
  async.auto({
    ...
    check: ['getAsset', function(callback, results) {
      ...
      // refresh not forced
      if(!options.force) {
        // if purchase validity period is now and asset hash has not changed
...
function _getKeywords(asset) {
  ...
  var keywords = text.replace(/[^A-Za-z0-9~\-_]/g, ' ').split(/\s/);
