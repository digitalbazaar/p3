...
api.init = function(app, callback) {
  ...
  async.waterfall([
    ...
    function(callback) {
      payswarm.mail.registerTrigger('fetchIdentity', function(event, callback) {
        api.getIdentity(
      ...
      payswarm.db.collections.identity.update(
        ...
        {$set: payswarm.db.buildUpdate(
          identity, 'identity', {exclude: [
            'identity.psaSlug', 'identity.psaStatus', 'identity.owner',
            'identity.preferences', 'identity.address']})},
...
api.getIdentityPreferences = function(actor, prefs, callback) {
  async.waterfall([
    ...
    function(record, callback) {
      if(record) {
        var prefs = record.identity.preferences;
        prefs['@context'] = payswarm.constants.CONTEXT_URL;
        record = prefs;
      }
      ...
      payswarm.db.collections.publicKey.update(
        ...
        {$set: payswarm.db.buildUpdate(
          publicKey, 'publicKey', {exclude: [
            'publicKey.psaStatus', 'publicKey.publicKeyPem',
            'publicKey.owner']})},
        payswarm.db.writeOptions,
        callback);
    },
...
api.revokeIdentityPublicKey = function(actor, publicKeyId, callback) {
  async.waterfall([
    ...
    function(key, callback) {
      ...
      key.psaStatus = 'disabled';
      key.revoked = payswarm.tools.w3cDate();
      payswarm.db.collections.publicKey.update(
        {id: payswarm.db.hash(publicKeyId), 'publicKey.psaStatus': 'active'},
        {$set: {
          'publicKey.psaStatus': key.psaStatus,
    ...
    function(n, key, callback) {
      if(n === 0) {
        return callback(new PaySwarmError(
          ...
          'revoked.', MODULE_TYPE + '.PublicKeyNotFound'));
      }
      callback(null, key);
    }
  ], callback);
  ...
  async.waterfall([
    ...
    function(callback) {
      payswarm.db.collections.publicKey.findOne(
        {owner: payswarm.db.hash(authorityId), 'publicKey.psaStatus': 'active'},
...
api.decryptMessage = function(actor, msg, decryptKey, callback) {
  ...
  async.auto({
    getDecryptKey: function(callback) {
      ...
      // encrypted with
      api.getIdentityPublicKey({id: msg.publicKey}, actor,
...
function _createIdentity(identity, callback) {
  ...
  } else if(identity.type === 'VendorIdentity') {
    identity = payswarm.tools.extend(
      true, {}, defaults.vendor, identity);
  } else {
    identity = payswarm.tools.extend(
