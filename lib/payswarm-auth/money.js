...
// special regex to use to work around bignumber.js not handling "d.".
var noFractionRegex = /^\d+\.$/;
...
api.Money = function(amount, precision, roundMode) {
  ...
  if(amount instanceof api.Money) {
    ...
    if(typeof amount === 'string' && noFractionRegex.test(amount)) {
      // change from 'ddd.' to 'ddd.0'
      amount = amount + '0';
    }
