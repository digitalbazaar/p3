...
// standard factory function for collecting money for an external system
// (eg: for fees for charging credit cards, bank accounts, etc)
api.createGenerousMoney = function(amount) {
  /* Note: When dealing with collecting money to bring into the system
    ensure enough is pulled from the external system to cover appropriate fees
    by always rounding up. Use 2 decimal places. */
  return new api.Money(amount, 2, api.Money.ROUND_MODE.UP);
};
// standard factory function for sending money to an external system
// (eg: for crediting credit cards, bank accounts, etc)
api.createStingyMoney = function(amount) {
