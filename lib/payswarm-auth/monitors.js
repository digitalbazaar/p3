...
function _initCubeMonitor() {
  ...
  var options = payswarm.tools.extend({}, bedrock.config.monitors.cube, {
    protocol: 'udp',
    host: 'localhost',
    port: 1180,
    debug: false
  ...
  bedrock.events.on('common.Transaction.settled', function(event) {
    client.send(_makeCubeTxnEvent(event, 'settled'));
  });
  bedrock.events.on('common.Transaction.voided', function(event) {
