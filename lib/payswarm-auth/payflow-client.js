...
api.PayflowClient = function(options) {
  ...
  this.agent = new https.Agent({
    // payflow API only supports SSLv3 (true story!)
 ...
 * @param callback(err, res) called once the operation completes.
 */
api.PayflowClient.prototype.charge = function(source, callback) {
  var self = this;
  var req = self.createChargeRequest(source);
...
api.PayflowClient.prototype.createVerifyRequest = function(source) {
  ...
  return _sourceToRequest(source, req);
};
 ...
 * @param res the response to the verification of the source.
 *
 * @return the PaymentToken.
 */
api.PayflowClient.prototype.createPaymentToken = function(source, res) {
  var token = null;
  if(res.response.RESPMSG === 'Verified') {
    token = {};
    token.paymentToken = res.response.PNREF;
    if(jsonld.hasValue(source, 'type', 'card:CreditCard')) {
      token.paymentMethod = 'card:CreditCard';
      ...
      token.bankRouting = source.routingNumber;
    }
  }
  return token;
};
...
/**
 * Writes a payment source into a request.
 *
 * @param source the payment source.
 ...
 * @return the request.
 */
function _sourceToRequest(source, req) {
  if(jsonld.hasValue(source, 'type', 'com:PaymentToken')) {
    _paymentTokenToRequest(source, req);
  }
  else if(jsonld.hasValue(source, 'type', 'card:CreditCard')) {
    _cardToRequest(source, req);
  }
  else if(jsonld.hasValue(source, 'type', 'bank:BankAccount')) {
    _bankAccountToRequest(source, req);
  }
  return req;
}
...
function _cardToRequest(card, req) {
  ...
  req.EXPDATE = card.cardExpMonth + card.cardExpYear;
