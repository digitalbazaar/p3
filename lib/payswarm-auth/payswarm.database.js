...
function _initLocalDatabase(callback) {
  ...
  async.waterfall([
    ...
    function(callback) {
      ...
      api.localClient.open(function connected(err, client) {
        if(!err) {
          payswarm.logger.info('connected to database: mongodb://' +
            payswarm.config.database.host + ':' +
            payswarm.config.database.port + '/' +
    ...
    function(callback) {
      ...
      api.localClient.createCollection(name, api.writeOptions,
        function ignoreAlreadyExists(err) {
          if(err && api.isAlreadyExistsError(err)) {
            err = null;
          }
    ...
    function(callback) {
      ...
      api.localClient.collection(name, api.readOptions, callback);
    },
    ...
    function(callback) {
      ...
      api.localCollection.insert(record, api.writeOptions,
        function(err) {
