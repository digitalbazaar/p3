...
// maximum number of sqlite busy retries
var MAX_SQLITE_RETRIES = 20;
 ...
 * @param callback(err) called once the operation completes.
 * @param retries the number of retries (when the db is busy) so far.
 */
api.setLocalItems = function(pairs, callback, retries) {
  async.waterfall([
    function(callback) {
    ...
    db.close();
    if(err) {
      // retry transaction when db is busy
      if(err.code === 'SQLITE_BUSY' && retries !== MAX_SQLITE_RETRIES) {
        if(retries === undefined) {
          retries = 0;
        }
        return process.nextTick(function() {
          api.setLocalItems(pairs, callback, retries + 1);
        });
      }
    }
    callback(err);
 ...
 * @param retries the number of retries (when the db is busy) so far.
 */
api.findAndModifyLocalItem = function(key, modifier, callback, retries) {
  var db;
  async.waterfall([
    function(callback) {
      // do not use cached database to ensure database is closed after
    ...
    if(err) {
      ...
      if(err.code === 'SQLITE_BUSY' && retries !== MAX_SQLITE_RETRIES) {
        ...
        return process.nextTick(function() {
          api.findAndModifyLocalItem(key, modifier, callback, retries + 1);
        });
      }
    }
    callback(err);
