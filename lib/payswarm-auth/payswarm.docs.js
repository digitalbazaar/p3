  ...
  for(var i in contentPaths) {
    for(var li in locales) {
      ...
      var docPath = path.resolve(
        path.join(contentPaths[i], lang, '/views/docs'));
      try {
        var stats = fs.statSync(docPath);
        if(stats) {
          paths[lang].push(docPath);
        }
      }
      catch(ex) {
    ...
    for(var p in paths[lang]) {
      ...
      fs.readdir(docPath, function(err, files) {
        ...
        for(var f in files) {
          ...
          if(path.extname(filename) === '.tpl' && filename !== 'index.tpl') {
            var rfilename = path.resolve(path.join(docPath, filename));
            _addDocsToIndex(docLang, rfilename);
          }
        }
      });
    }
...
api.getDocIndex = function(lang) {
  ...
  if(!docsMerged) {
        ...
        for(var docFile in docIndex[language][category]) {
          if(docFile in docDetails) {
            docIndex[language][category][docFile]['method'] =
              docDetails[language][docFile].method;
            docIndex[language][category][docFile]['path'] =
              docDetails[language][docFile].path;
          }
        }
      }
    }
    ...
    for(var language in docIndex) {
      for(var category in docIndex[language]) {
        for(var docFile in docIndex[language][category]) {
          if(!docIndex[language][category][docFile]['path']) {
            delete docIndex[language][category][docFile];
          }
        }
        if(Object.keys(docIndex[language][category]).length < 1) {
          delete docIndex[language][category];
        }
      }
    }
...
api.getDetails = function(lang, docFile) {
  return docDetails[lang][docFile];
};
...
api.document = function(method, path, docFile) {
  for(var lang in docDetails) {
    if(docFile in docDetails[lang]) {
      docDetails[lang][docFile].method = method;
      docDetails[lang][docFile].path = path;
    }
  }
};
...
function _addDocsToIndex(lang, filename) {
  fs.readFile(filename, 'utf-8', function(err, data) {
    ...
    docIndex[lang][category][shortName] = docs;
    docDetails[lang][shortName] = docs;

    // build the validation HTML if the validator exists
    if(validator) {
      var schema = payswarm.validation.getSchema(validator);
      validatorHtml = _generateValidatorHtml(schema);
...
function _serializeJsonSchemaString(prop, indent) {
  ...
  var maxLength = prop.maxLength ? prop.maxLength : '∞';
