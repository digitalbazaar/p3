...
function _serializeJsonSchemaObject(obj, indent) {
  var html = util.format('<div class="indent%i">%s: %s {</div>',
...
function _serializeJsonSchemaProperties(properties, indent) {
  ...
  for(p in properties) {
    prop = properties[p];
    html += util.format('<div class="indent%i"><strong>%s</strong>: %s - %s',
      p, prop.title, prop.description);
    if(prop.type === 'string') {
...
function _serializeJsonSchemaString(prop, indent) {
  ...
  html += '<div class="validator-string">';
  ...
  if(prop.pattern) {
    html += util.format(' regex(%s)', prop.pattern);
  }
  html += '</div>';
