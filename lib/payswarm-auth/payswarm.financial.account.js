...
api.updateAccount = function(actor, account, callback) {
 async.waterfall([
   ...
   function(callback) {
     ...
     delete account.psaStatus;
     delete account.balance;
     delete account.escrow;
...
function _createAccount(account, options, callback) {
  ...
  options = payswarm.tools.extend(true, {}, defaults.account, options);

  // create account
  account = payswarm.tools.extend(true, {}, options, account);

  // allow custom balance if configured
  var balance = null;
  if(payswarm.config.financial.allowCustomBalance) {
  ...
  account.escrow = new Money(0).toString();
