...
function _createAccount(account, callback) {
  ...
  var balance = null;
  // validate default balance
  if('balance' in defaults.account) {
    balance = new Money(defaults.account.balance);
    if(balance.isNegative()) {
      return callback(new PaySwarmError(
        'Default account balance must be positive.',
  ...
  async.waterfall([
    ...
    function(record, callback) {
      // if postive default balance, add a deposit
