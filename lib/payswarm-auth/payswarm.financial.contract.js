...
api.createContract = function(actor, options, callback) {
  ...
  async.auto({
    ...
    getAssetProvider: ['getAsset', function(callback, results) {
      ...
      payswarm.identity.getIdentity(null, id, function(err, identity) {
        ...
        var assetProvider = {
          ...
          'foaf:homepage': identity['foaf:homepage'],
          'dc:description': identity['dc:description']
        };
    ...
    var contract = {
      ...
      'dc:created': payswarm.tools.w3cDate(),
      'ps:listing': results.getListing,
      'ps:listingHash': options.listingHash,
      'ps:asset': results.getAsset,
      'ps:license': results.getLicense,
      'ps:assetProvider': results.getAssetProvider
    };
...
api.addPayeeToContract = function(contract, payee, options, callback) {
  ...
  var max = new Money(payee['com:rate']);
  ...
  for(var i in rules) {
    ...
    if(payswarm.tools.checkPayeeRule(rule, payee, options.ownerType)) {
      ...
      // update maximum rate
      if('com:maxmimumRate' in rule) {
        var tmp = new Money(rule['com:maximumRate']);
...
api.finalizeContract = function(actor, contract, options, callback) {
  ...
  async.auto({
    getAcquirer: function(callback) {
      ...
      if('vcard:adr' in options.acquirer) {
    ...
    checkSourceAccount: function(callback) {
      ...
      payswarm.financial.getAccount(
        actor, options.acquirerAccountId, function(err, account) {
          ...
          if(options.acquirer['@id'] !== account['ps:owner']) {
            return callback(new PaySwarmError(
    ...
    contract['ps:assetAcquirer'] = {
      '@id': results.getAcquirer['@id'],
      'vcard:adr': results.getAcquirer['vcard:adr']
    };
    ...
    // assign date
    contract['com:date'] = payswarm.tools.w3cDate();
    ...
    var record = {
      ...
      expires: Date.parse(contract['psa:expires']),
      meta: {
        created: now,
        updated: now
      },
    ...
    for(var i in payees) {
      ...
      if(dst in amountMap) {
        ...
        for(var ti in transfers) {
          ...
          if(dst === transfer['com:destination']) {
            var amount = transfer['com:amount'];
