...
api.processDeposit = function(actor, deposit, callback) {
  ...
  async.waterfall([
    ...
    function(callback) {
      ...
      if(actor) {
        // use actor id
        callback(null, actor, null);
      }
      else {
        var authId = payswarm.profile.createProfileId('authority');
        payswarm.profile.getProfile(null, authId, callback);
      }
    },
    function(emailProfile, meta, callback) {
      ...
      payswarm.profile.getProfile(
        actor, emailProfile.id, function(err, profile) {
    ...
    function(callback) {
      ...
      async.waterfall([
        function(callback) {
          gateway.chargeDepositSource(deposit, callback);
        }
      ], function(err, result) {
        if(err) {
