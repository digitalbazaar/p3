 ...
 * after signDeposit().
 *
 * @param req the HTTP request that originated the deposit. null if the request
 *          did not originate via HTTP.
 ...
 * @param callback(err, deposit) called once the operation completes.
 */
api.processDeposit = function(req, actor, deposit, callback) {
  ...
  async.auto({
    ...
    authorize: ['blind', function(callback, results) {
      ...
      payswarm.financial.authorizeTransaction(blinded, function(err) {
        // prepare data for the e-mail templates
        event.details.httpHeaders = null;
        if(req) {
          event.details.httpHeaders = '';
          for(header in req.headers) {
            event.details.httpHeaders +=
              header + ':' + req.headers[header] + '\n';
          }
        }
        event.details.profileJson =
          JSON.stringify(event.details.profile, null, 2);
        event.details.depositJson =
