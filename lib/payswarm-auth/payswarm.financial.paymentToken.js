 ...
 * BankAccount), if supported by the gateway given by token.paymentGateway. If
    ...
    function(records, callback) {
      ...
      async.forEachSeries(sources, function(source, callback) {
        ...
        api.createPaymentToken(
          actor, source, source.paymentGateway, token, function(err, token) {
            callback(err);
          });
      },
      function(err) {
...
function _generatePaymentTokenId(token) {
  token.id = util.format('%s/payment-token/%s',
    token.owner, encodeURIComponent(token.label));
}
