...
function _checkTransactionAccess(actor, transaction, callback) {
  ...
  console.log('AGAINST TRANSACTION', transaction);
  ...
  async.auto({
    ...
    isAssetAcquirer: ['getIdentities', function(callback, results) {
      ...
      if(transaction.assetAcquirer) {
        ...
        var i = 0;
        while(!isAssetAcquirer && i < records.length) {
          var identity = records[i].identity;
          if(identity.id === acquirer) {
            isAssetAcquirer = true;
          }
          i++;
        }
      }
    ...
    isAssetProvider: ['getIdentities', function(callback, results) {
      ...
      if(transaction.assetProvider) {
        ...
        var i = 0;
        while(!isAssetProvider && i < records.length) {
          var identity = records[i].identity;
          if(identity.id === provider) {
    ...
    checkAccess: ['isAssetAcquirer', 'isAssetProvider',
