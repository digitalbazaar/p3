...
function _voidTransaction(transaction, callback) {
  ...
  async.waterfall([
    function(callback) {
      ...
      query['outgoing.' + transactionHash] = {$exists: true};
      payswarm.db.collections.account.findOne(
...
function _runWorker(options, callback) {
  ...
  // FIXME: there's an issue with using max clean up count to determine if
  // a transaction should be cleaned up again ... if multiple workers run
  // (there will always be 1 per core), then the clean up count will reach
  // its maximum almost immediately, without any time passing to allow
  // transactions that are being updated by more than 1 process to finish
  // their work -- this means we probably also need an interval that needs
  // to pass before attempting the next cleanup (eg: compare against the
  // last update time once clean up ID > 0 ... but it may be the case that
  // we intentionally don't change the update time on cleanup and this won't
  // work)
