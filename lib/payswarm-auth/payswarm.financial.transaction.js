...
function _processTransaction(transactionId, callback) {
  ...
  async.waterfall([
    ...
    function(record, callback) {
      ...
      if(isDeposit) {
        return _checkDepositState(transaction, callback);
      }
      if(isWithdrawal) {
        return _checkWithdrawalState(transaction, callback);
      }
      callback();
    },
...
/**
 * Determines if a Deposit is ready to be settled by ensuring any related
 * monetary transactions performed by an external payment gateway have
 * reached an acceptable state. The acceptable state can vary by at least
 * the type of payment source used in the Deposit. For instance, if the
 * Deposit's payment source was a credit card (or a tokenized credit card),
 * then the Deposit is automatically considered settleable. If the payment
 * source is a bank account, an inquiry into a pending ACH transaction's
 * state may need to be made.
 *
 * @param transaction the Transaction (Deposit) to process.
 * @param callback(err) called once the operation completes.
 */
function _checkDepositState(transaction, callback) {
  // FIXME: implement
  // FIXME: run inquiry based on source type+gateway, if inquiry passes,
  // do not raise error, otherwise attempt to increase settlement time
  // by config amount if retries haven't been exhausted (also configurable)
  // raise an error in any case that the deposit can't be settled, but
  // print to log that a retry will happen in configuration time if that's
  // the case
  callback();
}

/**
 * Determines if a Withdrawal is ready to be settled by ensuring any related
 * monetary transactions performed by an external payment gateway have
 * reached an acceptable state. The acceptable state can vary by at least
 * the type of payment source used in the Withdrawal. For instance, if the
 * payment destination is a bank account, an inquiry into a pending ACH
 * transaction's state may need to be made.
 *
 * @param transaction the Transaction (Withdrawal) to process.
 * @param callback(err) called once the operation completes.
 */
function _checkWithdrawalState(transaction, callback) {
  // FIXME: implement
  callback();
}
