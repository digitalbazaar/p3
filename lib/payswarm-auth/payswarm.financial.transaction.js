  ...
  async.waterfall([
    ...
    function(callback) {
      ...
      payswarm.db.createIndexes([{
        ...
        // state+settleAfter+workers.start
        collection: 'transaction',
        fields: {state: 1, 'transaction.psaSettleAfter': 1, 'workers.start': 1},
        options: {unique: false, background: true}
      }], callback);
    },
...
function _runWorker(options, callback) {
  ...
  if(options.algorithm === 'settle') {
    ...
    4. There must be an expired worker and a state of authorized, processing,
      settling, or settled. */
    query = {'transaction.psaSettleAfter': {$lte: +new Date()}};
    if(options.id) {
      ...
      expiredQuery = {
        'transaction.psaSettleAfter': {$lte: +new Date()},
    ...
    async.waterfall([
      ...
      function(transactionId, override, callback) {
        ...
        payswarm.db.collections.transaction.update(
          ...
          {$pull: {workers: {$or: [{id: workerId}, {start: {$lte: past}}]}}},
          payswarm.db.writeOptions, function(err) {
