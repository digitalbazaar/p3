  ...
  async.auto({
    ...
    checkAccess: ['isAssetAcquirer', 'isAssetProvider', 'isSourceOwner',
      function(callback, results) {
        callback(null, results.isAssetAcquirer | results.isAssetProvider |
          results.isSourceOwner);
      }
    ],
  }, function(err, results) {
    callback(err, results.checkAccess);
  });
};
 ...
 * @returns true if a record match exists, false otherwise.
 */
function _idMatchExists(id, records, key) {
  var match = false;
  var i = 0;
  while(!match && i < records.length) {
