  ...
  async.waterfall([
    ...
    function(callback) {
      ...
      payswarm.db.createIndexes([{
        ...
        collection: 'transaction',
        fields: {'transaction.psaSettleAfter': 1, state: 1, 'meta.updated': 1},
        options: {unique: false, background: true}
      }, {
        // workerId+cleanups
        collection: 'transaction',
...
api.processTransaction = function(transaction, callback) {
  ...
  async.waterfall([
    ...
    function(record, callback) {
      ...
      // process transaction
      _processTransaction(transaction, record.settleId, callback);
    },
    function(callback) {
      // settle transaction
  ...
  async.waterfall([
    ...
    function(record, callback) {
      ...
      if(record.state === SETTLE_STATE.SETTLING) {
        ...
        return _settleTransaction(transaction, record.settleId, callback);
      }
      if(record.state === SETTLE_STATE.VOIDING ||
        record.state === SETTLE_STATE.VOIDED) {
        return callback(new PaySwarmError(
...
api.getTransactions = function(actor, query, fields, options, callback) {
  ...
  async.waterfall([
    ...
    // FIXME: remove once special db encoding is not necessary
    function(records, callback) {
      for(var i in records) {
        var record = records[i];
        if('transaction' in record) {
          record.transaction = payswarm.db.decode(record.transaction);
        }
      }
...
api.getTransaction = function(actor, id, callback) {
  async.waterfall([
    ...
    function(record, callback) {
      ...
      delete record.transaction.psaSettleAfter;
        ...
        async.waterfall([
          ...
          function(callback) {
            ...
            var record = {
              ...
              // FIXME: use special db encoding until storage is optimized
              // such that listings, assets, licenses, and any other
              // customizable data is not included in the transaction
              // collection, then this encoding should not be needed for the
              // transaction collection
              transaction: payswarm.db.encode(transaction)
            };
...
function _escrowDestinationFunds(
  ...
  async.waterfall([
    ...
    function(record, callback) {
      ...
      var oldSettleId = record.incoming[transactionHash];
      if(oldSettleId !== null) {
      ...
      var escrow = new Money(record.account.escrow);
  ...
  if(options.algorithm === 'settle') {
    /* To Settle:
       ...
       4. The last update must be in the past AND the state must be
         ...
         settled with either the workerId NOT set to '0' OR the number of
         cleanups less than the maximum. A workerId of '0' is
         a potential ending condition used to prevent indefinite
         clean up of settled transactions. Once a worker completes, it
         sets the workerId to zero. */
      ...
      query.$or = [{
        ...
        $or: [
          ...
          {state: SETTLE_STATE.SETTLING},
          {
            ...
            $or: [
              {workerId: {$ne: '0'}},
              {workerId: '0', cleanups: {$lt: maxCleanups}}
            ]
          }
        ]
      }];
    ...
    /* To Void:
     ...
     3. The last update must be in the past AND the state must be voiding OR
       the state must be voided with either the workerId NOT set to '0' OR
       the number of cleanups less than the maximum. A workerId of '0' is a
      ...
      async.until(function() {return done;}, function(callback) {
        async.waterfall([
          ...
          function(record, callback) {
            ...
            // FIXME: remove once special db encoding is not necessary
            // handle transaction
            var transaction = payswarm.db.decode(record.transaction);
            if(options.algorithm === 'settle') {
          ...
          function(transaction, finished, callback) {
            ...
            var update = {$set: {workerId: '0', 'meta.updated': +new Date()}};
            if(finished) {
              update.$inc = {cleanups: 1};
            }
            payswarm.db.collections.transaction.update(
              {id: payswarm.db.hash(transaction.id), workerId: workerId},
              update, payswarm.db.writeOptions, callback);
          }
        ], function(err) {
          callback(err);
