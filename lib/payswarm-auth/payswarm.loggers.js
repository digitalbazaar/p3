...
container.init = function() {
  if(cluster.isMaster) {
    ...
    // write to file transports to ensure they are created
    var fileTransports = [transports.app, transports.access, transports.error];
    for(var i in fileTransports) {
      var transport = fileTransports[i];
      // write as top-level (emergency) to ensure file is opened
      transport.log('emergency',
        'Logger transport initialized; log file opened.');
    }
