...
api.createPaymentToken = function(source, token, callback) {
  async.auto({
    ...
    if(jsonld.hasValue(source, 'type', 'ccard:CreditCard')) {
      ...
      token.bankRoutingNumber = blinded.banlRoutingNumber;
    }
    callback(null, token);
  });
};
...
api.authorizeDeposit = function(deposit, callback) {
  ...
  deposit.psaPaymentGatewayError = [];
  deposit.psaApproved = false;
  ...
  async.waterfall([
    function(callback) {
      ...
      if(jsonld.hasValue(source, 'type', 'ccard:CreditCard')) {
        ...
        return _createResponse(deposit, function(err, response) {
          if(err) {
            deposit.psaGatewayError.push('unprocessed');
            return callback(err);
          }
    ...
    payswarm.logger.debug(api.name,
      'gateway approved="' + deposit.psaApproved + '", errors:',
      deposit.psaGatewayError);

    if(!deposit.psaApproved) {
      err = new PaySwarmError(
        'Deposit authorization not approved.',
        MODULE_TYPE + '.DepositNotApproved',
        {errors: deposit.psaGatewayError}, err);
    }
    callback(err);
  });
};
