  ...
  async.waterfall([
    ...
    function(callback) {
      ...
      payswarm.db.createIndexes([{
        ...
        collection: 'profile',
        fields: {'profile.email': true},
        options: {unique: false, background: true}
      }, {
        collection: 'profile',
        fields: {'profile.psaSlug': true},
        options: {unique: true, background: true}
      }], callback);
    },
...
api.getProfile = function(actor, id, callback) {
  async.waterfall([
    ...
    function(result, callback) {
      ...
      delete result.profile.psaPassword;
      delete result.profile.psaPasscode;
...
function _createProfile(profile, callback) {
  ...
  payswarm.tools.extend(profile, {
    label: profile.label || profile.psaSlug,
