      ...
      function(callback) {
        payswarm.website.getDefaultViewVars(req, function(err, vars) {
          ...
          if(topic) {
            ...
            var details = payswarm.docs.getDetails(req.language, topic);
            vars.method = details.method;
            vars.path = details.path;
            res.render('docs/' + topic + '.tpl', vars);
          }
          else {
            vars.categories = payswarm.docs.getCategories(req.language);
            vars.docIndex = payswarm.docs.getDocIndex(req.language);
