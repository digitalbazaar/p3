...
function addServices(app, callback) {
  ...
  app.server.get('/i', ensureAuthenticated,
    function(req, res, next) {
      if(req.query.form === 'register' && req.query['public-key']) {
  ...
  async.waterfall([
    ...
    function(vars, callback) {
      ...
      if(req.query['public-key']) {
        vars.registrationType = "Public Key";
        vars.publicKey = req.query['public-key'];
      }
      if(req.query['public-key-label']) {
        vars.publicKeyLabel = req.query['public-key-label'];
      }
      else {
        vars.publicKeyLabel = 'Access Key';
      }
      if(req.query['registration-callback']) {
        vars.registrationType = "Vendor";
