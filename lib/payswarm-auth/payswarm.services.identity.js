      ...
      function(vars, records, callback) {
        ...
        for(var i in records) {
          ...
          if(account.psaStatus !== 'deleted') {
            // determine if current requestor is the owner of the account
            var isOwner = false;
            if(req.isAuthenticated()) {
              console.log(account, req.user.profile);
              isOwner = (vars.identity.owner === req.user.profile.id &&
                account.owner === vars.identity.id);
            }

            // show only public properties for the account
            if(!isOwner) {
              cleanedAccount = {'id': account.id};
              for(var j = 0; j < account.psaPublic.length; j++) {
                if(account[account.psaPublic[j]] !== undefined) {
                  cleanedAccount[account.psaPublic[j]] =
                    account[account.psaPublic[j]];
                }
              }
