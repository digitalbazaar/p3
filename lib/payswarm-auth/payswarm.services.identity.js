...
function addServices(app, callback) {
  ...
  app.server.post('/i', ensureAuthenticated,
    function(req, res, next) {
      var identity = {};
      identity['@id'] = payswarm.identity.createIdentityId(
        req.params['psa:slug']);
      identity['@type'] = req.body['@type'] || 'ps:PersonalIdentity';
      identity['ps:owner'] = req.user.profile['@id'];
      identity['psa:slug'] = req.body['psa:slug'];
      identity['rdfs:label'] = req.body['rdfs:label'];

      // only set homepage if provided
      if(req.body['foaf:homepage']) {
        identity['foaf:homepage'] = req.body['foaf:homepage'];
      }
      // only set description if provided
      if(req.body['dc:description']) {
        identity['dc:description'] = req.body['dc:description'];
      }
  ...
  app.server.post('/i/:identity', ensureAuthenticated,
    function(req, res, next) {
      // get ID from URL
    ...
    async.waterfall([
      ...
      function(vars, records, callback) {
        ...
        if(req.isAuthenticated()) {
          isOwner = (identity['ps:owner'] === req.user.profile['@id']);
        }
        ...
        for(var i in records) {
          ...
          if(account['psa:status'] !== 'deleted' &&
            (isOwner || account['psa:privacy'] === 'public')) {
  ...
  app.server.post('/i/:identity/preferences', ensureAuthenticated,
    function(req, res, next) {
      // get ID from URL
      var identityId = payswarm.identity.createIdentityId(req.params.identity);
      ...
      async.auto({
        getAccount: function(callback) {
          payswarm.financial.getAccount(
            req.user.profile, req.body['com:destination'],
        ...
        handleKey: function(callback) {
          // get existing key
          if(typeof req.body['sec:publicKey'] === 'string') {
            return payswarm.identity.getIdentityPublicKey(
              {'@id': req.body['sec:publicKey']}, function(err, key) {
                if(err) {
                  return callback(err);
                }
                callback(null, key);
          ...
          var key = {};
          key['@type'] = 'sec:Key';
          key['ps:owner'] = identityId;
          key['rdfs:label'] = req.body['sec:publicKey']['rdfs:label'];
          key['sec:publicKeyPem'] =
            req.body['sec:publicKey']['sec:publicKeyPem'];
        ...
        setPreferences: ['getAccount', 'handleKey',
          function(callback, results) {
            ...
            prefs['ps:owner'] = identityId;
            prefs['com:destination'] = account['@id'];
            prefs['sec:publicKey'] = key['@id'];
      ...
      async.waterfall([
        function(callback) {
          payswarm.identity.getIdentityPreferences(
            req.user.profile, {'ps:owner': identityId}, callback);
        },
