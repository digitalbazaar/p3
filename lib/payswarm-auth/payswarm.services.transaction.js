...
function addServices(app, callback) {
  ...
  app.server.get('/transactions/:id', ensureAuthenticated,
    ...
    async.auto({
      getTransaction: function(callback, results) {
        payswarm.financial.getTransaction(
          req.user.profile, transactionId, callback);
      },
      ...
      render: ['getTransaction', 'getVars', function(callback, results) {
        ...
        vars.transaction = results.getTransaction[0];
  ...
  app.server.get('/transactions', ensureAuthenticated,
    function(req, res, next) {
      ...
      if(req.query.view === 'activity') {
        return _getTransactionActivity(req, res, next);
      }
 ...
 * @param next the next handler.
 */
function _getTransactionActivity(req, res, next) {
  // FIXME: profile has no default identity, this is an error,
  // go to a page where they can create an identity
  if(!req.user.identity) {
    return res.redirect('/profile/settings');
...
function _getTransactions(req, res, next) {
  ...
  async.waterfall([
    ...
    function(records, callback) {
      ...
      records.forEach(function(record) {
        rset.resources.push(record.transaction);
      });
      res.json(rset);
    }
  ], function(err) {
