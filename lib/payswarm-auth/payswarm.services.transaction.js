...
function _processPurchaseRequest(req, res, next) {
  ...
  // vendor-initiated (budget-based) purchase request
  _processVendorPurchaseRequest(req, res, next);
}
...
/**
 * Handles a request to process a PurchaseRequest initiated by a vendor.
 *
 * @param req the request.
 * @param res the response.
 * @param next the next handler.
 */
function _processVendorPurchaseRequest(req, res, next) {
  // Vendor-initiated (budget-based) PurchaseRequest MUST be signed and
  // MUST contain:
  ...
  if(('nonce' in req.body) || ('callback' in req.body)) {
    return next(new PaySwarmError(
      ...
      'is functioning properly to avoid any potential security risk.',
  ...
  async.auto({
    ...
    processContract: ['getQuote', function(callback, results) {
      _processFinalizedContract(
        req.user.profile, {
          contract: results.getQuote,
          allowBudget: true,
    ...
    var contractId = results.processContract.id;
    res.set('Location', contractId);
    res.json(201, results.processContract);
  });
}
...
function _createDuplicateError(actor, options, callback) {
  ...
  payswarm.financial.getTransactions(
    actor, query, {transaction: true}, {limit: 1}, function(err, records) {
      ...
      _encryptShortContract(contract, options.nonce, function(err, encrypted) {
        if(err) {
          return callback(err);
        }
