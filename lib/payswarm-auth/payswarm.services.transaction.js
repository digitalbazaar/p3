...
function _processPartialPurchaseRequest(req, res, next) {
  ...
  async.auto({
    ...
    sendReceipt: ['encryptContract', function(callback, results) {
      // respond with encrypted short-form contract
...
function _emailReceipt(contract, callback) {
  ...
  async.waterfall([
    ...
    function(profile, meta, callback) {
      // send a purchase successful email to the buyer
      payswarm.events.emit('payswarm.common.Purchase.success-profile', {
        type: 'payswarm.common.Purchase.success-profile',
        details : {profile: profile, contract: contract}
      });

      // send a purchase success email to the service provider
