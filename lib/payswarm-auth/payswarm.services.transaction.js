  ...
  async.auto({
    getAcquirer: function(callback) {
      ...
      payswarm.financial.getAccount(
        req.user.profile, req.body.source, function(err, account) {
          if(err) {
            return callback(err);
          }
          callback(null, {id: account.owner});
        });
    },
    getListing: function(callback) {
      var query = {
        id: req.body.listing,
        hash: req.body.listingHash,
        type: 'ps:Listing',
        store: true,
        strict: true,
        fetch: true
    ...
    createQuote: ['handleDuplicate', function(callback, results) {
      ...
      var options = {
        ...
        acquirerAccountId: req.body.source
      };
      if('referenceId' in req.body) {
        options.referenceId = req.body.referenceId;
      }
  ...
  async.auto({
    ...
    getBudget: ['getContract', function(callback, results) {
      ...
      var query = {
        ...
        vendors: payswarm.db.hash(asset.assetProvider)
      };
      payswarm.financial.getBudgets(null, query, function(err, records) {
        ...
        if(records.length > 0) {
          ...
          if(budget.source !== transfers[0].source) {
            return callback(new PaySwarmError(
    ...
    updateBudget: ['getBudget', function(callback, results) {
      ...
      if(budget) {
        ...
        var amount = new Money(results.getContract.amount);
