...
function configureServer(app, callback) {
  ...
  var contentPaths = payswarm.config.website.content.views;
  var paths = [];

  // setup early handler to handle i18n-based URL re-writing
  // FIXME: Read cache in from disk
  var cache = {};
  app.server.earlyHandlers.push(function(req, res, next) {
    console.log("I18N cache handler!!!!!!!!!!!!");
    if(req.language) {
      var path = util.format('%s/%s', req.lang, req.path);
      if(cache[path]) {
        req.path = path;
      }
    }
  ...
  if(Array.isArray(contentPaths)) {
    for(var i in contentPaths) {
      ...
      paths.push(path.resolve(contentPaths[i]));
    }
  }
  else {
    paths = path.resolve(contentPaths);
  }
