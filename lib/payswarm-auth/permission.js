      ...
      async.forEachSeries(
        ...
        function(r, callback) {
          api.addRole(r, function(err) {
            if(err && err.name === MODULE_TYPE + '.DuplicateError') {
              err = null;
            }
            callback(err);
          });
...
api.addRole = function(role, callback) {
  ...
  if(role.id in roles) {
    return callback(new PaySwarmError(
      ...
      MODULE_TYPE + '.DuplicateError', {id: role.id}));
  }
...
api.updateRole = function(actor, role, callback) {
  ...
  async.waterfall([
    ...
    function(callback) {
      ...
      var copy = payswarm.tools.clone(roles[role.id]);
      if('label' in role) {
      ...
      if('psaPermission' in role) {
        copy.psaPermission = role.psaPermission;
      }
      roles[role.id] = copy;
      callback();
    }
...
api.checkRolePermission = function(role, permissions, callback) {
  async.waterfall([
    ...
    function(callback) {
      api.checkPermission(role.psaPermission, permissions, callback);
    }
  ], callback);
};
