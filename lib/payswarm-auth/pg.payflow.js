  ...
  async.waterfall([
    function(callback) {
      try {
        ...
        var cfg = bedrock.config.payflow;
...
api.createPaymentToken = function(source, token, callback) {
  async.auto({
    ...
    finish: ['hashSource', 'verifySource', function(callback, results) {
      ...
      token = bedrock.util.extend(token, res.paymentToken);
...
api.chargeDepositSource = function(deposit, callback) {
  ...
  async.waterfall([
    ...
    function(res, callback) {
      ...
      deposit.sysGatewayApprovalCode = res.response.AUTHCODE;
      deposit.sysGatewayRefId = res.response.PNREF;
...
api.holdDepositFunds = function(deposit, callback) {
  ...
  async.waterfall([
    ...
    function(res, callback) {
      logger.debug(api.name + ', gateway hold result',
        {response: res, deposit: deposit});
      result.response = res;
...
api.captureDepositFunds = function(deposit, options, callback) {
  ...
  async.waterfall([
    ...
    function(res, callback) {
      logger.debug(api.name + ', gateway capture result',
        {response: res, deposit: deposit});
      result.response = res;
...
api.creditWithdrawalDestination = function(withdrawal, amount, callback) {
  ...
  async.waterfall([
    ...
    function(res, callback) {
      ...
      result.approved = true;
      // add auth approval code for receipt logs
      withdrawal.sysGatewayApprovalCode = res.response.AUTHCODE;
      withdrawal.sysGatewayRefId = res.response.PNREF;
...
api.getTransactionStatus = function(txn, callback) {
  ...
  if(isDeposit) {
    ...
    if(held) {
      ...
      return api.captureDepositFunds(txn, function(err, res) {
        var result = {
          ...
          response: res,
          errors: [],
          settleAfterIncrement: bedrock.config.financial.paymentGateway.Payflow
    ...
    // consider card deposits without approved 'hold' to be
    // externally settled
    var src = txn.source;
...
api.addDepositPayees = function(deposit, callback) {
  ...
  var cfg =
    bedrock.config.financial.paymentGateway.Payflow.payees.deposit.CreditCard;
...
api.adjustDepositPrecision = function(deposit, callback) {
  ...
  if(roundedTotal.compareTo(total) > 0) {
    var roundingAccount = bedrock.config.financial.paymentGateway.Payflow
...
/**
 * Converts a payflow client error to a BedrockError.
 *
