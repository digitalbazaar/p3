...
api.chargeDepositSource = function(deposit, callback) {
  ...
  async.waterfall([
    ...
    if(!result.approved) {
      ...
      // set settlement to 48 hours from now
      deposit.psaSettleAfter = +new Date() + 1000*60*60*24;
    }
    callback(err, result);
  });
};
...
api.getTransactionStatus = function(txn, callback) {
  ...
  // compare settleAfter to created, if 48 hours have passed
  var created = +new Date(txn.created);
  var settleAfter = +new Date(txn.psaSettleAfter);
  var diff = settleAfter - created;
