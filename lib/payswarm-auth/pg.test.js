...
api.getTransactionStatus = function(txn, callback) {
  ...
  if(txn.psaStatusChecks === 0) {
    // add 24 hours to settleAfter to simulate slow txn on first check
    settleAfter += 1000*60*60*24;
  }
  if(+new Date() >= settleAfter) {
    return callback(null, {status: 'settled'});
  }
...
api.addDepositPayees = function(deposit, callback) {
  // FIXME: check first payee amount if necessary (to calculate

  // append gateway payees
  var cfg = payswarm.config.financial.paymentGateway.Test;
...
function _createResponse(deposit, callback) {
  ...
  var source = deposit.source;
  if(jsonld.hasValue(source, 'type', 'ccard:CreditCard')) {
    var address = source.cardAddress;
