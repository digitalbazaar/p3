  ...
  async.waterfall([
    function(callback) {
      try {
        ...
        var cfg = bedrock.config.usaepay;
...
api.createPaymentToken = function(source, token, callback) {
  async.auto({
    ...
    finish: ['hashSource', 'tokenize', function(callback, results) {
      ...
      token = bedrock.util.extend(token, paymentToken);
      token.sysTokenHash = results.hashSource;
      token.paymentGateway = api.gatewayName;
      token.sysVerified = false;
      token.sysVerifyReady = false;
      callback(null, token);
    }]
...
api.chargeDepositSource = function(deposit, callback) {
  ...
  async.waterfall([
    ...
    function(res, callback) {
      logger.debug(api.name + ', gateway charge result',
      ...
      if(res.ResultCode === 'E') {
        ...
        deposit.sysGatewayApprovalCode = res.AuthCode;
        deposit.sysGatewayRefId = res.RefNum;
...
api.creditWithdrawalDestination = function(withdrawal, amount, callback) {
  ...
  async.waterfall([
    ...
    function(res, callback) {
      logger.debug(api.name + ', gateway credit result',
      ...
      if(res.ResultCode === 'E') {
        ...
        result.approved = true;
        // add auth approval code for receipt logs
        withdrawal.sysGatewayApprovalCode = res.AuthCode;
        withdrawal.sysGatewayRefId = res.RefNum;
    ...
    if(result.approved &&
      ...
      withdrawal.sysSettleAfter = (+new Date() +
        bedrock.config.financial.paymentGateway.USAePay.bankAccountSettlement);
    }
...
api.getTransactionStatus = function(txn, callback) {
  ...
  api.client.inquire(txn, function(err, res) {
    var result = {
      ...
      response: res,
      errors: [],
      settleAfterIncrement: bedrock.config.financial.transaction
    ...
    case 'N':
    // Pending For cards, batch hasn't closed yet. For checks, hasn't
    ...
    case 'F':
      ...
      break;
    // Settled For cards batch has been closed and transaction has
...
api.addDepositPayees = function(deposit, callback) {
  ...
  var cfg = bedrock.config.financial.paymentGateway.USAePay.payees.deposit;
...
api.addWithdrawalPayees = function(withdrawal, callback) {
  // append gateway payees
  var cfg = bedrock.config.financial.paymentGateway.USAePay.payees.withdrawal;
...
api.adjustDepositPrecision = function(deposit, callback) {
  ...
  if(roundedTotal.compareTo(total) > 0) {
    var roundingAccount = bedrock.config.financial.paymentGateway.USAePay
