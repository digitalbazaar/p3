...
api.init = function(app, callback) {
  ...
  async.waterfall([
    ...
    function(callback) {
      payswarm.mail.registerTrigger('fetchProfile', function(event, callback) {
        api.getProfile(
...
api.resolveProfileIdentifier = function(identifier, callback) {
  ...
  if(identifier.indexOf('@') !== -1) {
    ...
    payswarm.identity = require('./identity');
    payswarm.identity.getIdentities(
      null, {'identity.psaSlug': identifier}, {'identity.owner': true},
      ...
      payswarm.db.collections.profile.update(
        ...
        {$set: payswarm.db.buildUpdate(
          profile, 'profile', {exclude: [
            'profile.psaSlug', 'profile.psaPassword', 'profile.psaPasswordNew',
            'profile.psaPasscode', 'profile.psaRole', 'profile.psaStatus']})},
...
function _updateProfilePassword(profile, callback) {
  ...
  async.auto({
    ...
    hashPassword: ['checkPassword', 'checkPasscode',
      function(callback, results) {
        ...
        if('psaPasswordNew' in profile) {
          payswarm.security.createPasswordHash(
            profile.psaPasswordNew, callback);
        } else {
          callback();
        }
    }],
    generatePasscode: ['hashPassword', function(callback, results) {
      ...
      payswarm.security.createPasswordHash(passcode, callback);
    }]
...
api.verifyProfileEmail = function(actor, profile, callback) {
  async.waterfall([
    ...
    function(verified, callback) {
      ...
      payswarm.db.collections.profile.update(
        {id: payswarm.db.hash(profile.id)},
        {$set: {'profile.psaEmailVerified': true}},
...
function _verifyProfilePasswordHash(profile, type, callback) {
  ...
  async.waterfall([
    ...
    function(hash, callback) {
      payswarm.security.verifyPasswordHash(hash, profile[field], callback);
    },
    function(verified, legacy) {
      ...
      payswarm.security.createPasswordHash(profile[field], function(err, hash) {
        var update = {$set: {}};
        update.$set['profile.' + field] = hash;
        payswarm.db.collections.profile.update(
          {id: payswarm.db.hash(profile.id)}, update,
          payswarm.db.writeOptions,
          function(err) {
            callback(err, verified);
          });
  ...
  async.waterfall([
    ...
    function(roles, callback) {
      ...
      roles.forEach(function(role) {
        role.psaPermission.forEach(function(permission) {
...
function _populateRoles(roleIds, callback) {
  payswarm.permission.getRoles(roleIds, callback);
}
...
function _createProfile(profile, callback) {
  async.waterfall([
    function(callback) {
      ...
      payswarm.db.collections.profile.findOne(
        {'profile.psaSlug': profile.psaSlug}, {'profile.psaSlug': true},
        function(err, record) {
          if(err) {
            return callback(err);
          }
      ...
      async.auto({
        hashPassword: function(callback) {
          if(profile.psaHashedPassword === true) {
            ...
            payswarm.security.createPasswordHash(profile.psaPassword, callback);
          }
        },
