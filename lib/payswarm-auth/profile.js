...
api.resolveProfilename = function(name, callback) {
  payswarm.db.collections.profile.findOne(
    {$or: [{id: payswarm.db.hash(name)}, {'profile.sysSlug': name}]},
    {'profile.id': true},
    function(err, result) {
      if(!err && result) {
 ...
 * has no 'id' or 'sysSlug' property or either of those properties is not
      ...
      async.auto({
        hashPassword: function(callback) {
          if(profile.sysHashedPassword === true) {
            ...
            payswarm.security.createPasswordHash(profile.sysPassword, callback);
          }
        },
        ...
        // an ID was given
        var result = null;
        var generateId = !(profile.id && profile.sysSlug);
        ...
        async.until(function() {return result !== null;}, function(callback) {
          ...
          if(generateId) {
            profile.sysSlug = payswarm.tools.uuid();
