...
function _validateListing(listing, callback) {
  async.auto({
    ...
    verify: ['getAsset', 'getListingKey', function(callback, results) {
      ...
      var vendors = jsonld.getValues(asset, 'vendor');
      if(vendors.length > 0 && !jsonld.hasValue(asset, 'vendor', vendor)) {
        return callback(new PaySwarmError(
          'Invalid Listing; the Listing\'s vendor is prohibited from selling ' +
          'the Asset.',
          MODULE_TYPE + '.ProhibitedVendor'));
      }

      // ensure listing public key is asset provider's, vendor's, or authority
      var assetProvider = asset.assetProvider;
      var key = results.getListingKey;
      if(key.owner !== assetProvider && key.owner !== vendor &&
        key.owner !== payswarm.config.authority.id) {
        return callback(new PaySwarmError(
          'Invalid Listing; the public key used to verify the Listing ' +
          'is not owned by the Asset provider, the vendor, or a PaySwarm ' +
          'Authority.',
          MODULE_TYPE + '.InvalidListing', {id: listing.id}, err));
      }
      // verify signature
