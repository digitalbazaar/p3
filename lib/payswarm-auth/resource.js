...
function _openResourceCollections(options, callback) {
  async.eachSeries(options, function(opts, callback) {
...
function _openResourceCollection(name, callback) {
  async.waterfall([
    function(callback) {
      brDatabase.openCollections([name], callback);
    },
    function(callback) {
...
ResourceStorage.prototype.get = function(query, callback) {
  ...
  logger.debug('ResourceStorage.get,', {
    collection: self.name, query: query
  });

  // build mongo query
  var q = {id: brDatabase.hash(query.id)};
  if(query.hash) {
    q.hash = query.hash;
  }
  var options = {};
  ...
  async.waterfall([
    function(callback) {
      ...
      brDatabase.collections[self.name].find(
    ...
    function(records, callback) {
      ...
      records.forEach(function(record) {
        record.resource = brDatabase.decode(record.resource);
      });
...
ResourceStorage.prototype._fetch = function(query, callback) {
  ...
  async.waterfall([
    ...
    function(data, callback) {
      logger.debug('ResourceStorage fetched data,', {
        collection: self.name, query: query
      });
    ...
    function(data, hash, callback) {
      ...
      // build record
      var record = {
        id: brDatabase.hash(query.id),
        hash: hash,
        date: now,
        // encode reserved mongodb characters, use @context URL
        resource: bedrock.util.extend(
          brDatabase.encode(data),
      ...
      logger.debug('ResourceStorage storing data,', {
        collection: self.name,
        query: query
      });

      // store record
      brDatabase.collections[self.name].insert(
        record, brDatabase.writeOptions, function(err, result) {
          ...
          if(brDatabase.isDuplicateError(err)) {
            return brDatabase.collections[self.name].find(
    ...
    function(callback) {
      ...
      jsdom.env({
        ...
        url: query.id,
          ...
          if(errors) {
            logger.debug('ResourceStorage fetch errors:', errors);
            return callback(new BedrockError(
...
function _validateAsset(asset, callback) {
  async.waterfall([
    function(callback) {
      brValidation.validate('resources.asset', asset, callback);
    },
    function(callback) {
      // get public key that signed
      var id = asset.signature.creator;
      brIdentity.getIdentityPublicKey({id: id}, function(err, key) {
        if(err) {
          return callback(new BedrockError(
    ...
    function(key, callback) {
      ...
      if(key.owner !== asset.assetProvider &&
        key.owner !== bedrock.config.authority.id) {
        return callback(new BedrockError(
...
function _validateLicense(license, callback) {
  brValidation.validate('resources.license', license, callback);
}
...
function _validateListing(listing, callback) {
  async.auto({
    validate: function(callback) {
      brValidation.validate('resources.listing', listing, callback);
    },
    ...
    verify: ['getAsset', 'getListingKey', function(callback, results) {
      ...
      if(key.owner !== vendor && key.owner !== bedrock.config.authority.id) {
        return callback(new BedrockError(
          'Invalid Listing; the public key used to verify the Listing ' +
