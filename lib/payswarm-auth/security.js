...
api.signJsonLd = function(obj, key, creator, nonce, date, callback) {
  ...
  async.waterfall([
    function(callback) {
      ...
      _getSignatureData(obj, function(err, data, obj) {
  ...
  async.waterfall([
    ...
    function(compacted, key, iv, callback) {
      try {
        ...
        compacted['@context'] = payswarm.tools.getDefaultJsonLdContextUrl();
 ...
 * @param callback(err, data, obj) called once the operation completes.
 */
function _getSignatureData(obj, callback) {
  ...
  _compact(obj, function(err, input) {
    ...
    jsonld.normalize(input, options, function(err, normalized) {
      ...
      callback(null, normalized, input);
    });
  });
}
...
function _compact(input, callback) {
  // if context is not present in input, it is safe to assume it uses
  // the default payswarm context
  if(!('@context' in input)) {
    var compacted = payswarm.tools.clone(input);
    compacted['@context'] = payswarm.tools.PAYSWARM_CONTEXT;
    return callback(null, compacted);
  }

  // always compact (ensure consistency w/expression of payswarm context or
  // convert from another context)
