      ...
      async.waterfall([
        ...
        function(account, meta, callback) {
          ...
          if('psaAllowInstantTransfer' in req.body) {
            account.psaAllowInstantTransfer = req.body.psaAllowInstantTransfer;
          }
          if('psaMinInstantTransfer' in req.body) {
            account.psaMinInstantTransfer = req.body.psaMinInstantTransfer;
          }
          // if psaAllowInstantTransfer true, must have psaMinInstantTransfer
          if(req.body.psaAllowInstantTransfer &&
            !('psaMinInstantTransfer' in req.body)) {
            account.psaMinInstantTransfer = Money.ZERO.toString();
          }
          ...
          var updateBackupSource = function(err, account) {
            ...
            if(newSources.length === 0) {
              ...
              return payswarm.financial.removeAccountBackupSource(
                req.user.profile, accountId, oldSources[oldSources.length - 1],
                function(err) {
                  if(err) {
                    return callback(err);
                  }
            ...
            return payswarm.identity.getIdentity(
              req.user.profile, identityId, function(err, identity) {
                ...
                getDefaultViewVars(req, function(err, vars) {
                  ...
                  res.render('financial/account.html', vars);
                });
              });
          }*/
          var cleanedAccount =
...
function _getTransactionActivity(options, req, res, next) {
  async.auto({
    ...
    render: ['getAccount', 'getVars', function(callback, results) {
      ...
      res.render('transactions.html', vars);
    }]
  }, function(err) {
    if(err) {
      next(err);
