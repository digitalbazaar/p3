...
function _getTransactionActivity(options, req, res, next) {
  async.auto({
    ...
    render: ['getAccount', 'getVars', function(callback, results) {
      ...
      var cleanedAccount =
        _cleanAccount({identity: options.identity}, req, account);
...
function _cleanAccount(options, req, account) {
  ...
  if(account !== null) {
    console.log('cleaning account', options, req.user, account);
    if('user' in req &&
  ...
  console.log('cleaned account', cleanedAccount);
