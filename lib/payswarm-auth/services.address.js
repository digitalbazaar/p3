...
function addServices(app, callback) {
  ...
  app.server.delete('/i/:identity/addresses', ensureAuthenticated,
    function(req, res, next) {
      _deleteAddress(req, res, next);
    });
...
function _deleteAddress(req, res, next) {
  ...
  // FIXME: handle missing param
  var addressId = req.query.addressId;

  async.waterfall([
    function(callback) {
      // delete identity address
      payswarm.identity.deleteIdentityAddress(
        req.user.profile, id, addressId, callback);
    }
  ], function(err) {
    if(err) {
      err = new PaySwarmError(
