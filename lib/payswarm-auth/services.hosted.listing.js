...
function addServices(app, callback) {
  ...
  app.server.post('/i/:identity/receipts',
    ...
    function(req, res, next) {
      ...
      var identityId = payswarm.identity.createIdentityId(req.params.identity);
      var encryptedReceipt = req.body;

      var params = {};
      async.waterfall([
        function(callback) {
          // do not check permissions during decryption, checked below
          payswarm.identity.decryptMessage(null, encryptedReceipt, callback);
        },
        function(receipt, decryptKey, callback) {
          // *IMPORTANT* validate that the message is a receipt -- only this
          // kind of message is permitted to be decrypted by this service
        ...
        function(callback) {
          ...
          payswarm.identity.getIdentityPublicKey(
            {id: params.receipt.signature.created}, function(err, publicKey) {
              if(err) {
                return callback(err);
              }
        ...
        function(callback) {
          // ensure receipt was encrypted by authority or by a key owned
          // by the vendor
          if(params.decryptKey.owner !== identityId &&
            params.decryptKey.owner !== params.receipt.contract.vendor) {
          ...
          if(params.publicKey.owner !== payswarm.config.authority.id) {
            return callback(new PaySwarmError(
          ...
          if(params.receipt.contract.vendor !== identityId) {
            return callback(new PaySwarmError(
          ...
          payswarm.security.verifyJsonLd(
            params.receipt, params.publicKey, callback);
        },
        function(callback) {
          // get hosted asset and associated public key
          // FIXME: figure out how these public keys will be stored ...
          // might need an index based on asset content URL (map
          // directory to a public key)
          payswarm.hosted.asset.getAsset(null, assetId, callback);
        },
        function(asset, publicKey, callback) {
          // FIXME: decrypt receipt, check signature
          // confirm vendor is identity
          // get hosted asset (and associated public key)

          // FIXME: if there is no associated public key, just do a simple
          // redirect to the URL, a receipt isn't needed because there's
          // no protection scheme in place at the content URL

          // re-encrypt receipt
          // post encrypted receipt to asset content URL
          // FIXME: make generated file hit /i/foo/assets?assetContentUrl=...
          // or other URL and either return JSON and populate PHP page ... or
          // return a page with the asset and each hosted listing for it
        }
      ], function(err) {
        if(err) {
          err = new PaySwarmError(
