...
function addServices(app, callback) {
  ...
  app.server.get('/i/:identity/listings/:listing',
    ...
    function(req, res, next) {
      // get IDs from URL
      var identityId = payswarm.identity.createIdentityId(req.params.identity);
      var listingId = payswarm.listing.createId(identityId, req.params.listing);

      async.waterfall([
        function(callback) {
          payswarm.hostedListing.getListing(
            req.user.profile. listingId, callback);
        },
...
function _getListings(req, res, next) {
  ...
  if(req.query.keywords) {
    var keywords = _.uniq(req.query.keywords.split(/\s/).filter(function(e) {
      return e.length > 0;
    }));
  ...
  async.waterfall([
    ...
    function(records, callback) {
      ...
      records.forEach(function(record) {
        ...
        listing['@context'] = payswarm.tools.getDefaultJsonLdContextUrl();
        if(excludeAsset) {
