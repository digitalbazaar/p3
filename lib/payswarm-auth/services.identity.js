        ...
        payswarm.identity.getIdentity(
          null, identityId, function(err, identity, meta) {
            ...
            var isOwner = req.isAuthenticated() &&
              privateIdentity.owner === req.user.profile.id;
            if(isOwner) {
              vars.identity = privateIdentity;
            }
            else {
              // only include public info
              var publicIdentity = {id: identity.id, type: identity.type};
        ...
        records.forEach(function(record) {
          ...
          if(account.psaStatus !== 'deleted') {
            ...
            if(isOwner) {
              ...
              var publicAccount = {id: account.id, type: account.type};
