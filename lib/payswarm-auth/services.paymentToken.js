...
// module API
var api = {};
api.name = MODULE_NS + '.paymentToken';
...
function addServices(app, callback) {
  ...
  app.post('/i/:identity/payment-tokens',
    ensureAuthenticated,
    ...
    function(req, res, next) {
      ...
      async.waterfall([
        function(callback) {
          ...
          var identityId =
            brIdentity.createIdentityId(req.params.identity);
        ...
        res.status(201).json(record.paymentToken);
      });
  });

  app.get('/i/:identity/payment-tokens', ensureAuthenticated,
  ...
  app.get('/i/:identity/payment-tokens/:paymentToken',
    ensureAuthenticated,
    function(req, res, next) {
  ...
  app.post('/i/:identity/payment-tokens/:paymentToken',
    ensureAuthenticated,
  ...
  app.delete('/i/:identity/payment-tokens/:paymentToken',
    ensureAuthenticated,
    function(req, res, next) {
      // get IDs from URL
      var identityId = brIdentity.createIdentityId(req.params.identity);
      ...
      payswarm.financial.removePaymentToken(
        req.user.identity, paymentTokenId, function(err, record) {
          ...
          if(record) {
            return res.send(record.paymentToken);
          }
          res.status(204).end();
        });
  });
