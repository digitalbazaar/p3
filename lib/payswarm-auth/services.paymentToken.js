  ...
  app.server.post('/i/:identity/payment-tokens/:paymentToken',
    ...
    function(req, res, next) {
      ...
      if(req.query.action === 'verify') {
        console.log('PT POST V', req.body);
        return validate('services.paymentToken.postVerify')(
          req, res, function(err) {
            if(err) {
              return next(err);
            }
            _verifyPaymentToken(req, res, next);
          });
      }
...
function _verifyPaymentToken(req, res, next) {
  // get IDs from URL
  var identityId = payswarm.identity.createIdentityId(req.params.identity);
  var tokenId = payswarm.financial.createPaymentTokenId(
    identityId, req.params.paymentToken);

  // if input is not a deposit, create one and sign it for review
  if(!jsonld.hasValue(req, 'type', 'com:Deposit')) {
    // get verify parameters
    var amounts = req.body.psaVerifyParameters.amounts;
    ...
    return async.waterfall([
      ...
      function(callback) {
        ...
        var authAccount = config.financial.paymentTokenVerifyAccount;
        amounts.forEach(function(amount) {
          jsonld.addValue(deposit, 'payee', {
            ...
            comment: 'Recoup withdrawal verification amount'
          });
        });
        ...
        payswarm.financial.signDeposit(
          req.user.profile, deposit, {verify: true}, function(err, signed) {
            if(err) {
              return next(err);
            }
