...
// module API
var api = {};
api.name = MODULE_NS + '.transaction';
...
function addServices(app, callback) {
  ...
  app.get('/transactions/:transaction',
    ...
    brRest.makeResourceHandler({
      get: function(req, res, callback) {
        // get txn ID from URL
        var transactionId = payswarm.financial.createTransactionId(
  ...
  app.get('/transactions', ensureAuthenticated,
...
function _process(err, process, req, res, next, options) {
  ...
  var checkAuthentication = brPassport.checkAuthentication;
  var ensureAuthenticated = brPassport.ensureAuthenticated;
...
function _processDeposit(req, res, next) {
  ...
  if(!('signature' in req.body)) {
    ...
    return async.waterfall([
      ...
      signed['@context'] = payswarm.constants.PAYSWARM_CONTEXT_V1_URL;
      res.json(signed);
    });
  }
  ...
  async.auto({
    ...
    processContract: ['getContract', function(callback, results) {
      var options = {
        ...
        allowBudget: ('allowBudget' in req.body) ? req.body.allowBudget : true,
        requireBudget: false
      };
      if('nonce' in req.body) {
        options.nonce = req.body.nonce;
...
function _processSignedPurchaseRequest(req, res, next) {
  ...
  async.waterfall([
    ...
    function(_signer, callback) {
      signer = _signer;
      options.signerActor = signer.identity;
    ...
    function(contract, callback) {
      ...
      _processFinalizedContract(
        acquirerActor, {
          contract: contract,
          allowBudget: ('allowBudget' in req.body ?
            req.body.allowBudget : true),
...
function _getPaymentForm(req, res, next) {
  ...
  async.waterfall([
    ...
    function(vars, callback) {
      // create data for UI
      vars.identity = req.user.identity.id;
      vars.listing = req.query.listing;
      vars.listingHash = req.query['listing-hash'];
      vars.allowDuplicatePurchases =
        vars.paymentDefaults.allowDuplicatePurchases;

      // optional data
      if('reference-id' in req.query) {
        vars.referenceId = req.query['reference-id'];
      }
      if('callback' in req.query) {
        vars.callback = req.query.callback;
      }
      if('response-nonce' in req.query) {
        vars.nonce = req.query['response-nonce'];
      }
      if('referer' in req.headers) {
        vars.referer = req.headers.referer;
      }
...
function _getTransactions(req, res, next) {
  ...
  async.waterfall([
    function(callback) {
      if(options.account) {
        // retrieve account to ensure its owned by identity and valid, etc.
    ...
    function(account, callback) {
      if(account) {
        query.accounts = brDatabase.hash(account.id);
      } else {
        // use identity
        ...
        query.identities = brDatabase.hash(req.user.identity.id);
      }
      ...
      if(options.previous !== null) {
        opts.min = {};
        if(options.account) {
          opts.min.accounts = brDatabase.hash(options.account);
        } else {
    ...
    function(records, callback) {
      ...
      records.forEach(function(record) {
        ...
        var contextUrl = payswarm.constants.PAYSWARM_CONTEXT_V1_URL;
        if('asset' in txn) {
 ...
 *          signerActor the identity of the entity that signed (authorized)
  ...
  async.auto({
    ...
    processContract: ['updateBudget', function(callback, results) {
      ...
      if('referenceId' in contract) {
        query.referenceId = brDatabase.hash(contract.referenceId);
      } else {
        // do asset look up
        query.asset = brDatabase.hash(contract.asset.id);
      }
...
function _getBudget(acquirer, vendor, callback) {
  var query = {
    owner: brDatabase.hash(acquirer),
    vendors: brDatabase.hash(vendor)
  };
 ...
 *
 * @param actor the Identity performing the check.
...
function _checkDuplicate(actor, options, callback) {
  ...
  if('referenceId' in options) {
    // do reference ID look up
    query.referenceId = brDatabase.hash(options.referenceId);
  } else if('asset' in options) {
    // do asset look up
    query.asset = brDatabase.hash(options.asset);
...
function _createDuplicateQuery(assetAcquirer) {
  ...
  return {
    assetAcquirer: brDatabase.hash(assetAcquirer),
...
function _emailContract(contract, callback) {
  ...
  // FIXME: emails should be optional and rate limited via per-identity settings
  var assetAcquirer = contract.assetAcquirer.id;

  async.waterfall([
    function(callback) {
      brIdentity.getIdentity(null, assetAcquirer, callback);
    },
...
function _createReceipt(contract, options, callback) {
  ...
  jsonld.frame(contract, frame, function(err, framed) {
    ...
    if(options.nonce !== null) {
      ...
      return payswarm.authority.encryptMessage(
    ...
    async.waterfall([
      function(callback) {
        ...
        payswarm.authority.getAuthorityKeyPair(
      ...
      function(keypair, callback) {
        ...
        payswarm.security.signJsonLd(receipt, {
          key: privateKey,
