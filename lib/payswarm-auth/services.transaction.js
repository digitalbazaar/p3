    ...
    function(account, callback) {
      ...
      if(query) {
        ...
        if(options.previous !== null) {
          // limit max key (dates are descending, so max key is used instead
          // of min key)
          opts.max = {};
          if(options.account) {
            opts.max.accounts = payswarm.db.hash(options.account);
          }
          else {
            opts.max.identity = query.identity;
          }
          opts.max.created = options.createdStart;
          opts.max.id = payswarm.db.hash(options.previous);
        }
        payswarm.financial.getTransactions(
...
function _emailReceipt(contract, callback) {
  ...
  async.waterfall([
    ...
    function(profile, meta, callback) {
      ...
      payswarm.events.emit('common.Purchase.success', {
