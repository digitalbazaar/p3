...
function addServices(app, callback) {
  ...
  app.server.post('/transactions', ensureAuthenticated,
    ...
    function(req, res, next) {
      ...
      var txn = req.body;
      var isContract = jsonld.hasValue(txn, 'type', 'com:Contract');
      var isDeposit = jsonld.hasValue(txn, 'type', 'com:Deposit');
      var isWithdrawal = jsonld.hasValue(txn, 'type', 'com:Withdrawal');
      var isTransaction = jsonld.hasValue(txn, 'type', 'com:Transaction');
  ...
  app.server.get('/transactions/:id', ensureAuthenticated, function(req, res, next) {
    ...
    async.auto({
      ...
      render: ['getTransaction', 'getVars', function(callback, results) {
        ...
        else if(jsonld.hasValue(vars.transaction, 'type', 'com:Withdrawal')) {
          vars.transfers = vars.transaction.transfer;
      ...
      if(query) {
        ...
        if(options.previous !== null) {
          ...
          if(options.account) {
            ...
            opts.min.identity = query.identity;
          }
...
function _checkDuplicate(actor, options, callback) {
  var query = _createDuplicateQuery(options.identity);
 ...
 *
 * @param identityId the Identity to use.
 */
function _createDuplicateQuery(identityId) {
...
function _createReceipt(contract, options, callback) {
  ...
  jsonld.frame(contract, frame, function(err, framed) {
    ...
    var preferences = [];
    if(options.budget) {
      preferences.push('ps:preAuthorization');
    }
    var receipt = {
