  ...
  async.waterfall([
    ...
    function(account, callback) {
      ...
      if(query) {
        ...
        var profile = req.user.profile.query;
        // skip permission check if identity is used; identity *must* have
        // access to its own transactions
        if(query.identities) {
          profile = null;
        }
        payswarm.financial.getTransactions(
          profile, query, {transaction: true}, opts, callback);
      }
      else {
        callback(null, []);
      }
