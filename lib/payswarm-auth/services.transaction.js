...
var ensureAuthenticatedOptions = payswarm.website.ensureAuthenticatedOptions;
 ...
 *          signedGraphEnabled true to check JSON-LD signed graphs [false]
 */
function _process(err, process, req, res, next, options) {
  ...
  if(options) {
    ensureAuthenticatedOptions(req, res, function(err) {
      if(err) {
        return next(err);
      }
...
function addServices(app, callback) {
    ...
    function(req, res, next) {
      ...
      if(jsonld.hasValue(req.body, 'type', 'PurchaseRequest')) {
        return validate('services.transaction.postPurchaseRequest')(
          req, res, function(err) {
            _process(err, _processPurchaseRequest, req, res, next, {
              signedGraphEnabled: true
            });
          });
      }
  ...
  app.server.get('/transactions/:id', ensureAuthenticated,
    ...
    async.auto({
      ...
      compact: ['transaction', function(callback, results) {
        jsonld.compact(results.transaction, payswarm.tools.CONTEXT_URL,
      ...
      render: ['compact', 'getVars', function(callback, results) {
        function ldjson() {
          res.json(results.compact);
        }
        res.format({
          ...
          html: function() {
            ...
            vars.transaction = results.compact;
...
function _createDuplicateError(actor, options, callback) {
  ...
  payswarm.financial.getTransactions(
    actor, query, {transaction: true}, {limit: 1}, function(err, records) {
      ...
      contract['@context'] = payswarm.tools.getDefaultJsonLdContextUrl();
