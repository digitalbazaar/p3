...
function setupMocha() {
  ...
  if(process.env.TEST_ENV.indexOf('integration') > -1) {
    var itDir = path.join(__dirname, 'tests', 'integration');
  ...
  } else if(process.env.TEST_ENV.indexOf('integration') > -1) {
    mocha.run(function(failures) {
...
function runTests() {
  if(!process.env.TEST_ENV) {
    ...
    console.log('TEST_ENV must contain either \'unit\', \'integration\' or ' +
  ...
  if(process.env.TEST_ENV === 'unit') {
    ...
    if(!path.existsSync(seleniumJar) || !path.existsSync(chromeDriver)) {
    ...
    agent.get('http://localhost:4444/').end(function(err, status) {
      if(!err || status !== undefined) {
        console.log('Found existing Selenium server. ' +
          ' Cannot start integration testing Selenium server.');
        process.exit(1);
      }
