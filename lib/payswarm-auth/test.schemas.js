...
module.exports = {
  ...
  'when validating a paymentTokens POST': {
    ...
    'build valid content': function(schema) {
      ...
      post['@context'] = 'http://purl.org/payswarm/v1';
      assert.isFalse(jsonschema(post, schema).valid);
      ...
      // wrong type
      post.source.type = 'ccard:CreditCard';
      assert.isFalse(jsonschema(post, schema).valid);

      // additional type
      post.source.type = ['bank:BankAccount', 'ccard:CreditCard'];
      assert.isFalse(jsonschema(post, schema).valid);

      post.source.type = 'bank:BankAccount';
