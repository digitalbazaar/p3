...
var payswarm = {
  money: require(__libdir + '/payswarm-auth/money', true),
  tools: require(__libdir + '/payswarm-auth/tools', true)
};
...
describe('payswarm.money', function() {
  ...
  describe('negate()', function() {
    it('should work for positive money tests', function(done) {
      var m = new Money('1');
      m.isNegative().should.be.false;
      m.negate().isNegative().should.be.true;
      done();
    });
    it('should work for negative money tests', function(done) {
      var m = new Money('-1');
      m.isNegative().should.be.true;
      m.negate().isNegative().should.be.false;
      done();
    });
  });
