...
PAYEE.APPLY_TYPE = {
  ...
  flat amount rates, take an equal part out of each applicable Payee to get to
  the total amount. For percent rates, take the rate as a percentage from each
  applicable Payee to get the total amount; this is like a typical income
  tax. */
...
function _resolvePayeeFlatInclusive(payee) {
  // divide the rate by the total number of dependencies to get a multiplier
  var multiplier = new Money(payee.payeeRate);
  multiplier = multiplier.divide(payee._deps.length);

  // iterate over each payee dependency applying the rate by adding it to
  // a total and decrementing it from the payee's amount
  var total = new Money(0);
