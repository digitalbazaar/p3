...
function _toObject(err, options) {
  ...
  if(options['public']) {
    ...
    if(err instanceof api.PaySwarmError && err.details['public']) {
  ...
  payees.forEach(function(payee) {
    ...
    if(deps.length === 0) {
      ...
      // percent payee
        ...
        // use all groups - member groups - exempt groups
        deps = _.keys(groups);
        deps = _.difference(jsonld.getValues('payee', 'payeeGroup'));
        deps = _.difference(jsonld.getValues('payee', 'payeeExemptGroup'));
      }
    }

    if(deps) {
      // get unique payees
      deps = _(groups).pick(payee._deps).values().flatten().uniq().value();
    }
...
function _resolvePercentExclusive(payee) {
...
function _resolvePercentInclusive(payee) {
  // get percentage rate as a multiplier
  var multiplier = new Money(payee.payeeRate);
  multiplier = multiplier.divide(100);
