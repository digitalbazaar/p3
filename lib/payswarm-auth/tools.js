...
api.blindBankAccount = function(account) {
  ...
  // show last 3 digits or as many as are available, fill rest with '*'
  account.bankAccount = account.bankAccount.replace(/.{1}(?=.{3})/g, '*');
...
api.appendPayees = function(p1, p2) {
  ...
  p2.forEach(function(payee) {
    api.appendPayee(p1, payee);
  });
};
