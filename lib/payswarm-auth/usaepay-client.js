...
api.USAePayClient.prototype.tokenize = function(account, callback) {
  ...
  token.bankAccount = account.bankAccount.replace(/.{1}(?=.{3})/g, '*');
  token.bankAccountType = account.bankAccountType || 'bank:Checking';
  // FIXME: mask routing number?
  token.bankRoutingNumber = account.bankRoutingNumber;
  ...
  async.waterfall([
    function(callback) {
      var args = {
        Token: _createSecurityToken(self.options.sourceKey),
        CustomerData: {
 ...
 * @param callback(err, acount) called once the operation completes.
 */
api.USAePayClient.prototype.getBankAccount = function(token, callback) {
  var self = this;
  var split = token.paymentMethod.split(':');
