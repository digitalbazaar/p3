 ...
 * - signed JSON-LD graph
 *
 * If both the session and HTTP signature methods are present, their associated
 * profile and identity must match. If the signed graph method is present, it
 * will override the other methods.
 *
...
api.ensureAuthenticated = function(req, res, next) {
  ...
  async.auto({
    ...
    signedGraph: function(callback) {
      ...
      if(!payswarm.config.website.authentication.signedGraph.enabled) {
        return callback(null, {
          user: false
        });
      }
