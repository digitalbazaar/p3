...
api.ensureAuthenticated = function(req, res, next) {
  ...
  async.auto({
    ...
    auth: ['check', 'signedGraph', function(callback, results) {
      // use signedGraph is present, otherwise the session or httpSignature
      callback(null, {
...
function configureServer(app, callback) {
  ...
  app.server.param(':asset', api.payswarmIdParam);
  ...
  app.server.param(':listing', api.payswarmIdParam);
