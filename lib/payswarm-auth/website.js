 ...
 * @param callback callback(err, credentials) called with error or null and the
 *          found credentials as {profile: ..., identity: ...} or false
 */
...
api.ensureAuthenticated = function(req, res, next) {
  api.checkAuthentication(req, function(err, credentials) {
    if(err) {
      return callback(err);
    }
