...
function configureServer(app, callback) {
  ...
  async.forEach(payswarm.config.website.i18nPaths, function(base, callback) {
    var prefix = '/';
    ...
    walker.on('file', function(root, stat, next) {
      ...
      var reqPath = path.join(prefix, dir, stat.name);
      i18nCache[language][reqPath] = path.join(prefix, language, dir, stat.name);
      next();
    });
