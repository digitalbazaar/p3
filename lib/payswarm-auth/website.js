...
/**
 * Ensures a request has been authenticated. Currently supported methods:
 *
 * - website session based on user and password
 * - HTTP signature: https://github.com/joyent/node-http-signature
 * - signed JSON-LD graph (optional)
 *
 * If both the session and HTTP signature methods are present, their associated
 * profile and identity must match. If the signed graph method is enabled and
 * a valid signed graph is present, the authenticated user will be the signer.
 *
 * @param req the request.
 * @param res the response.
 * @param next the next route handler.
 * @param options: (optional)
 *          signedGraphEnabled check signed graph singature [false].
 */
api.ensureAuthenticatedOptions = function(req, res, next, options) {
  ...
  async.auto({
    ...
    httpSignature: function(callback) {
      ...
      if(!payswarm.config.website.authentication.httpSignature.enabled) {
        return callback(null, {
          user: false
        });
      }
      // do http signature authentication check
      passport.authenticate('payswarm.httpSignature',
    ...
    signedGraph: function(callback) {
      // check if signed graph authentication enabled
      if(!options || !options.signedGraphEnabled) {
        return callback(null, {
          user: false
        });
      }
      // do signed graph authentication check
    ...
    check: ['session', 'httpSignature', function(callback, results) {
      // check session and httpSignature match if both present
      var session = results.session.user;
      var httpSignature = results.httpSignature.user;
      ...
      callback(null, {
        user: results.session.user || results.httpSignature.user
      });
    }],
    // select final authorization
    auth: ['check', 'signedGraph', function(callback, results) {
      // use signedGraph if present, otherwise the session or httpSignature
      callback(null, {
        user: results.signedGraph.user || results.check.user
      });
    }]
...
/**
 * Uses ensureAuthenticatedOptions to ensure a request has been authenticated.
 * This method does not allow options to enable Signed Graph checking.
 *
 * @param req the request.
 * @param res the response.
 * @param next the next route handler.
...
api.ensureAuthenticated = function(req, res, next) {
  return api.ensureAuthenticatedOptions(req, res, next, {});
}
...
function configureServer(app, callback) {
  ...
  passport.use(new payswarm.HttpSignatureStrategy());
